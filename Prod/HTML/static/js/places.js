const ohsites = [{name:"Adelaide Hunter Hoodless Homestead ",location:[43.23694,-80.29667],Date:"1839 (completed)",Designated:"1995",City:"Brant County",Type:"NHistoric",Description:"The childhood home of activist and organizer Adelaide Hunter Hoodless, educational reformer and co-founder of the Women's Institute, the National Council of Women of Canada and the Victorian Order of Nurses"},
{name:"Algoma Central Engine House ",location:[46.5281611,-84.3505389],Date:"1912 (completed)",Designated:"1992",City:"Sault Ste. Marie",Type:"NHistoric",Description:"A well-preserved example of a brick engine house, and the first in Canada to have an internal turntable"},
{name:"Algonquin Provincial Park ",location:[45.58417,-78.35833],Date:"1893 (established)",Designated:"1992",City:"Nipissing - Unorganized South Part (primarily)",Type:"NHistoric",Description:"The first provincial park in Canada, noted for its pioneering role in park management, visitor interpretation programs and the development of park buildings and structures, as well as its role in inspiring artists such as the Group of Seven"},
{name:"Amherstburg First Baptist Church ",location:[42.1026833,-83.1057361],Date:"1849 (completed)",Designated:"2012",City:"Amherstburg ",Type:"NHistoric",Description:"A modest wooden church that was a major Underground Railroad church in Upper Canada and that played a crucial role in the development of the Black community in Ontario"},
{name:"Amherstburg Navy Yard ",location:[42.1018833,-83.1125583],Date:"1796 (established)",Designated:"1928",City:"Amherstburg ",Type:"NHistoric",Description:"The site of a Royal Navy shipyard from 1796â1813, which served as the hub of the British naval presence on the upper Great Lakes"},
{name:"Annandale House/Tillsonburg Museum ",location:[42.86250,-80.72167],Date:"1882 (completed)",Designated:"1997",City:"Tillsonburg",Type:"NHistoric",Description:"One of the best surviving illustrations in Canada of the Aesthetic Movement and the movement's major impact on domestic architecture in Canada"},
{name:"Backhouse Grist Mill ",location:[42.6427000,-80.4748000],Date:"1798 (completed)",Designated:"1998",City:"Norfolk County",Type:"NHistoric",Description:"One of the few gristmills in this region not to be burned during the War of 1812, it is one of the oldest and best preserved examples in Canada of small-scale, water-powered establishments found throughout the country in the 19th and early 20th centuries"},
{name:"Banting House ",location:[42.9899583,-81.2317944],Date:"1900 (completed)",Designated:"1997",City:"London",Type:"NHistoric",Description:"A yellow-brick house recognized as the site of the defining moment in the discovery of insulin by Frederick Banting"},
{name:"Barnum House ",location:[43.9944361,-78.015889],Date:"1820 (completed)",Designated:"1959",City:"Grafton",Type:"NHistoric",Description:"A noted example of Neoclassic domestic architecture as brought to Canada by settlers from New England"},
{name:"Battle Hill ",location:[42.69417,-81.70500],Date:"1814 (battle)",Designated:"1924",City:"Southwest Middlesex",Type:"NHistoric",Description:"The site of the Battle of Longwoods during the War of 1812"},
{name:"Battle of Crysler's Farm ",location:[44.9419778,-75.0701722],Date:"1813 (battle)",Designated:"1920",City:"South Dundas",Type:"NHistoric",Description:"The site of a victory by badly outnumbered British troops in the War of 1812, prompting the American forces to abandon the St. Lawrence Campaign; the original battle site was submerged in 1958 by the construction of the Saint Lawrence Seaway, so the 1895 monument was relocated to its current location in Crysler's Farm Battlefield Park near Upper Canada Village"},
{name:"Battle of the Windmill ",location:[44.7209000,-75.4871000],Date:"1838 (battle)",Designated:"1920",City:"Prescott",Type:"NHistoric",Description:"The site a battle fought during the Upper Canada Rebellion, whereby Loyalist forces defeated an invasion attempt by Hunter Patriot insurgents based in the United States"},
{name:"Beausoleil Island ",location:[44.8661000,-79.8699000],Date:"nan",Designated:"2011",City:"Muskoka District",Type:"NHistoric",Description:"An island within Georgian Bay Islands National Park; a cultural landscape of the Anishinaabeg"},
{name:"Beechcroft and Lakehurst Gardens ",location:[44.2733111,-79.5030250],Date:"1870 (ca.) (established)",Designated:"1978",City:"Roches Point",Type:"NHistoric",Description:"Gardens reportedly designed by Frederick Law Olmsted"},
{name:"Bell Homestead ",location:[43.107917,-80.2708361],Date:"1858 (completed)",Designated:"1996",City:"Brantford",Type:"NHistoric",Description:"The first home of Alexander Graham Bell in North America, it is associated with the conception of and early long-distance trials of the telephone"},
{name:"Belle Vue ",location:[42.093222,-83.1127750],Date:"1819 (completed)",Designated:"1959",City:"Amherstburg",Type:"NHistoric",Description:"A two-storey, white painted, brick house constructed for the Deputy Assistant Commissary General of the garrison at Fort Malden, ranking among the finest examples of Palladian architecture in Canada"},
{name:"Belleville Railway Station (Grand Trunk) ",location:[44.1790028,-77.3747361],Date:"1856 (completed)",Designated:"1973",City:"Belleville",Type:"NHistoric",Description:"A railway station representative of the larger stations erected for the newly formed Grand Trunk Railway along the key Toronto to Montreal line during the mid-19th century, it is the oldest continuously operating passenger train station in Canada"},
{name:"Bethune Memorial House ",location:[44.9204028,-79.3761361],Date:"1880 (completed)",Designated:"1996",City:"Gravenhurst",Type:"NHistoric",Description:"The birthplace of Dr. Norman Bethune"},
{name:"Bethune-Thompson House / White House ",location:[45.1437972,-74.5747889],Date:"1805 (completed)",Designated:"1966",City:"Williamstown",Type:"NHistoric",Description:"A noted early Ontario home, representative of the design and construction techniques from the period; portions date to the 1780s when Loyalist Peter Ferguson settled on the site, but the main structure was built in 1805 as a manse for Reverend John Bethune, the first Presbyterian minister in Upper Canada and was later the residence of explorer David Thompson"},
{name:"Billy Bishop Boyhood Home ",location:[44.56639,-80.947639],Date:"1884 (completed)",Designated:"2002",City:"Owen Sound",Type:"NHistoric",Description:"The birthplace and childhood home of First World War flying ace Billy Bishop"},
{name:"Bois Blanc Island Lighthouse and Blockhouse ",location:[42.0878250,-83.1198556],Date:"1837 (lighthouse completed), 1839 (blockhouse completed)",Designated:"1955",City:"Bois Blanc Island",Type:"NHistoric",Description:"The site of the attack by Canadian rebels and their American supporters in January 1838 during the Upper Canada Rebellion; in response to the raid, the blockhouse was built to complement the reconstructed Fort Malden"},
{name:"Bridge Island / Chimney Island ",location:[44.4686417,-75.8345028],Date:"1814 (blockhouse completed)",Designated:"1936",City:"Front of Yonge",Type:"NHistoric",Description:"Formerly the site of a naval station during the War of 1812"},
{name:"Buxton Settlement ",location:[42.305667,-82.220417],Date:"1849 (established)",Designated:"1999",City:"Chatham-Kent",Type:"NHistoric",Description:"A community founded by abolitionist Reverend William King, 15 former American slaves, and an association which included James Bruce, 8th Earl of Elgin, then the Governor General of Canada, to create a haven for fugitive slaves escaping via the Underground Railroad"},
{name:"Canadian Car & Foundry ",location:[48.3570472,-89.3056722],Date:"1912 (completed)",Designated:"2009",City:"Thunder Bay",Type:"NHistoric",Description:"Located within the Bombardier Transportation facility, the historic complex was the main plant of Canada's largest aircraft manufacturer during the Second World War, with 10% of the world's production of the Hurricane built there; representative of the wartime contributions made by women workers and of the country's post-war mass-transit manufacturing industry"},
{name:"Canal Lake Concrete Arch Bridge ",location:[44.5583083,-79.0461167],Date:"1905 (completed)",Designated:"1988",City:"Bolsover",Type:"NHistoric",Description:"A bridge spanning the Trent-Severn Waterway, it is the earliest known reinforced concrete bridge in Canada"},
{name:"Carrying Place of the Bay of Quinte ",location:[44.0485472,-77.5828750],Date:"1787 (treaty negotiated)",Designated:"1929",City:"Carrying Place",Type:"NHistoric",Description:"The location where Sir John Johnson and Chiefs of the Mississauga negotiated a treaty ceding a river and portage route between Lake Ontario and Lake Huron"},
{name:"Castle Kilbride ",location:[43.40417,-80.67139],Date:"1877 (completed)",Designated:"1993",City:"Baden",Type:"NHistoric",Description:"An Italianate villa, the former residence of the (Flax and Oil King of Canada) James Livingston, known for its interior decorative mural paintings"},
{name:"Chiefswood ",location:[43.1010778,-80.0949667],Date:"1856 (completed)",Designated:"1953",City:"Six Nations of the Grand River First Nation",Type:"NHistoric",Description:"Italianate-style birthplace of poet E. Pauline Johnson"},
{name:"Christ Church, Her Majesty's Chapel Royal of the Mohawk ",location:[44.1855361,-77.0734056],Date:"1843 (completed)",Designated:"1995",City:"Deseronto",Type:"NHistoric",Description:"A historic chapel linked with establishment of Mohawk nation in Ontario; one of only two Royal chapels in Canada, representing the historic alliance between the British Crown and the Mohawk peoples"},
{name:"Claverleigh ",location:[44.3180139,-80.1021778],Date:"1871 (completed)",Designated:"1990",City:"Creemore",Type:"NHistoric",Description:"A noted example of a Gothic Revival-style wood villa"},
{name:"Cliff Site ",location:[42.7855917,-80.1963111],Date:"1670 (event)",Designated:"1919",City:"Port Dover",Type:"NHistoric",Description:"Marked by a large memorial cross, the site where two Sulpician priests, FranÃ§ois Dollier de Casson and RenÃ© de BrÃ©hant de GalinÃ©e, laid claim to the north shore of Lake Erie in the name of France"},
{name:"Cobalt Mining District ",location:[47.3975944,-79.6743528],Date:"1903 (established)",Designated:"2002",City:"Cobalt",Type:"NHistoric",Description:"A cultural landscape comprising buildings and structures associated with early 20th-century silver mining and urban settlement, once one of the largest silver producing areas in the world"},
{name:"Cox Terrace ",location:[44.3037667,-78.326222],Date:"1884 (completed)",Designated:"1991",City:"Peterborough",Type:"NHistoric",Description:"A noted example of a residential terrace built in the Second Empire style"},
{name:"Cummins Pre-contact Site ",location:[48.40583,-89.35333],Date:"nan",Designated:"1981",City:"Thunder Bay",Type:"NHistoric",Description:"A site once on the shoreline of glacial Lake Minong; extensive late Paleo-Indian quarry"},
{name:"Darlingside ",location:[44.368283,-75.969317],Date:"1830 (established)",Designated:"1992",City:"Leeds and the Thousand Islands",Type:"NHistoric",Description:"Rare surviving example of the wood depots that sold fuel to the steamboats that plied the St. Lawrence River and the Great Lakes"},
{name:"Donaldson Site ",location:[44.5057556,-81.3340417],Date:"500 BC - 300 AD",Designated:"1982",City:"Chippewa Hill",Type:"NHistoric",Description:"A 1.2-hectare (3.0-acre) archaeological site; the largest and best documented Saugeen Complex site known, representing many habitation and mortuary practices of the Woodland period"},
{name:"Ermatinger House ",location:[46.5062583,-84.3246278],Date:"1823 (completed)",Designated:"1957",City:"Sault Ste. Marie",Type:"NHistoric",Description:"A stone house believed to be the oldest surviving house in Northwestern Ontario, it was built by Charles Oakes Ermatinger, an active partner of the North West Company, and was used as a temporary headquarters by Garnet Wolseley, 1st Viscount Wolseley during the Red River Expedition"},
{name:"Etharita Site ",location:[44.412000,-80.210583],Date:"1647â49",Designated:"1982",City:"Clearview",Type:"NHistoric",Description:"The main village of the Wolf Tribe of the Petun"},
{name:"Fairfield on the Thames ",location:[42.6321583,-81.8697306],Date:"1792 (established), 1813 (destroyed)",Designated:"1945",City:"ChathamâKent",Type:"NHistoric",Description:"The site of the original village of Fairfield, founded in 1792 by Aboriginal refugees and Moravian missionaries who came to Canada from Ohio, fleeing persecution in the United States after refusing to take sides during the American Revolutionary War; the village was destroyed in 1813 by American invaders during the War of 1812, when the inhabitants were accused of sheltering British officers"},
{name:"Finnish Labour Temple ",location:[48.43222,-89.23000],Date:"1910 (completed)",Designated:"2011",City:"Thunder Bay",Type:"NHistoric",Description:"A two-storey brick building that played an important social and community role for Finnish immigrants to Canada"},
{name:"First Commercial Oil Field ",location:[42.783417,-82.1167722],Date:"1858 (discovery of oil)",Designated:"1925",City:"Oil Springs",Type:"NHistoric",Description:"The site of the first commercial oil well in the world, the first drilled well in Canada, the first gumbeds that were commercially used in the world, and the first gas gusher in Canada"},
{name:"Forbes Textile Mill ",location:[43.4275111,-80.3201611],Date:"1863 (established)",Designated:"1989",City:"Cambridge",Type:"NHistoric",Description:"A wool mill that was, for a time in the early decades of the twentieth century, the largest woollen and worsted mill in Canada"},
{name:"Former Almonte Post Office ",location:[45.2255417,-76.1951417],Date:"1891 (completed)",Designated:"1983",City:"Almonte",Type:"NHistoric",Description:"A stone post office with a steep gabled roof and central clock tower; designed by Thomas Fuller, the building has undergone no major exterior alterations, so remains an excellent representative example of early multi-use federal buildings in small communities"},
{name:"Former Bowmanville Boys Training School/Camp 30 ",location:[43.92722,-78.66750],Date:"1924 (established)",Designated:"2013",City:"Bowmanville",Type:"NHistoric",Description:"A group of Prairie-style buildings originally constructed as a progressive boys school, and used during the Second World War as an internment camp for German prisoners of war"},
{name:"Former Brockville Post Office ",location:[44.5898111,-75.6848833],Date:"1886 (completed)",Designated:"1983",City:"Brockville",Type:"NHistoric",Description:"A stone post office, blending Flemish, Queen Anne and classical elements; a good example of the post offices erected by the Department of Public Works in smaller urban centres during Thomas Fuller's term as Chief Dominion Architect"},
{name:"Former Elora Drill Shed ",location:[43.680056,-80.4288917],Date:"1865 (completed)",Designated:"1989",City:"Elora",Type:"NHistoric",Description:"A good representative example of the early stage in drill hall construction in Canada (when rural militia units, rather than the Department of Defence, were responsible for their construction), noted for its classical proportions, the fanlight over the door and the oculus in the gable"},
{name:"Former Galt Post Office ",location:[43.3583556,-80.3155528],Date:"1887 (completed)",Designated:"1983",City:"Cambridge",Type:"NHistoric",Description:"A limestone-clad post office, blending elements of Romanesque Revival, Gothic Revival and Second Empire architectural styles; representative of small urban post offices designed by Thomas Fuller"},
{name:"Former Port Perry Town Hall ",location:[44.1032972,-78.9472667],Date:"1873 (completed)",Designated:"1984",City:"Port Perry",Type:"NHistoric",Description:"A noted example of a municipal meeting hall; the lower hall was used for village council meetings, and the balconied opera house on the second storey served as the village's social centre"},
{name:"Fort Malden ",location:[42.1077111,-83.1126444],Date:"1799 (Fort Amherstburg completed), 1815 (fort initially rebuilt)",Designated:"1921",City:"Amherstburg",Type:"NHistoric",Description:"British fort (initially known as Fort Amherstburg) that served as the principal defence of the western frontier for the period until 1813 (when it was captured and later destroyed by the Americans), and also served as an important fortification during the border raids associated with the Upper Canada Rebellion in 1837-38"},
{name:"Fort Norfolk (Turkey Point)",location:[42.6994056,-80.324972],Date:"1814 (completed)",Designated:"1925",City:"Norfolk County",Type:"NHistoric",Description:"The site of a British military and naval post between 1814 and 1815; abandoned shortly after the War of 1812"},
{name:"Fort Sainte Marie II ",location:[44.8241611,-80.1642333],Date:"1649 (established)",Designated:"1920",City:"Christian Island",Type:"NHistoric",Description:"A Jesuit mission to the Huron-Wendat 1649-50; in 1651, remaining Huron-Wendat made their last stand against the Iroquois from this mission before fleeing to Quebec"},
{name:"Fort St. Joseph ",location:[46.06333,-83.94667],Date:"1796 (established)",Designated:"1923",City:"St. Joseph Island",Type:"NHistoric",Description:"Built as a counterpoint to an American garrison on Mackinac Island, Fort St. Joseph was the British Empire's most westerly outpost; destroyed by the Americans in 1812 when British forces left to take Fort Michilimackinac; the ruins of the fortifications and the archaeological resources on the site reveal the complex aspects of military, domestic and commercial life (both Aboriginal and European) in a frontier outpost"},
{name:"Fort St. Pierre ",location:[48.6183861,-93.3601000],Date:"1731 (established)",Designated:"1934",City:"Fort Frances",Type:"NHistoric",Description:"The first French fort built west of Fort Kaministiquia by Pierre La VÃ©rendrye in Northwestern Ontario"},
{name:"Fort Wellington ",location:[44.7129000,-75.508500],Date:"1813 (established)",Designated:"1920",City:"Prescott",Type:"NHistoric",Description:"One of the best preserved nineteenth-century fortifications in Canada, the fort protected shipping along the St. Lawrence River during War of 1812"},
{name:"Fort William ",location:[48.34278,-89.35833],Date:"1803 (established)",Designated:"1923",City:"Thunder Bay",Type:"NHistoric",Description:"Important North West Company post, now serving as a reconstructed living history site"},
{name:"FranÃ§ois BÃ¢by House ",location:[42.318500,-83.0424000],Date:"1812 (completed)",Designated:"1950",City:"Windsor",Type:"NHistoric",Description:"During the War of 1812, American forces crossed the Detroit River and used the house as headquarters for their invasion; when the Americans retreated one month later, the BÃ¢by House was occupied by British forces under Major-General Isaac Brock, who built an artillery battery on the property and used it to open fire on Fort Detroit"},
{name:"Fulford Place ",location:[44.5972889,-75.6708250],Date:"1901 (completed)",Designated:"1992",City:"Brockville",Type:"NHistoric",Description:"An excellent intact example of the type of mansion erected by wealthy Canadians in the late 19th and early 20th centuries, with the general layout of the site remaining as it was originally laid out by the Olmsted Brothers"},
{name:"Gillies Grove and House ",location:[45.4449889,-76.3589500],Date:"1937 (completed)",Designated:"1993",City:"Arnprior",Type:"NHistoric",Description:"House associated with two of the most prominent forest industry families in the Ottawa Valley, the McLachlins and the Gillies; surrounded by one of the few remaining accessible woodlots containing significant stands of old growth Ottawa Valley White Pine"},
{name:"Glanmore / Phillips-Faulkner House ",location:[44.1669750,-77.3675361],Date:"1883 (completed)",Designated:"1969",City:"Belleville",Type:"NHistoric",Description:"House designed by architect Thomas Hanley for J.P.C Phillips, a wealthy Belleville banker and financier; an excellent representative example of the Second Empire style popular among the upper middle class in late 19th-century Canada"},
{name:"Glengarry Cairn ",location:[45.1217194,-74.4898139],Date:"1842 (completed)",Designated:"1921",City:"South Glengarry",Type:"NHistoric",Description:"Large cairn erected by the Glengarry militia to commemorate the services of Sir John Colborne, commander-in-chief of the armed forces during the Upper Canada Rebellion"},
{name:"Glengarry House ",location:[45.03944,-74.6190972],Date:"1792 (completed)",Designated:"1921",City:"Cornwall",Type:"NHistoric",Description:"The ruins of the residence of Lieutenant Colonel John Macdonell, a pioneer in the settlement of Ontario, first Speaker of the Legislative Assembly of Upper Canada, and a hero of the Battle of Queenston Heights"},
{name:"Glengarry Landing ",location:[44.4518278,-79.9000667],Date:"1814 (flotilla construction)",Designated:"1923",City:"Edenvale",Type:"NHistoric",Description:"Site at the junction of the Nottawasaga River and Marl Creek, where in 1814 the Glengarry Light Infantry Fencibles, under the command of Lieutenant-Colonel Robert McDouall, constructed a flotilla of boats to relieve the British garrison at Fort Michilimackinac and to effect the subsequent capture of Prairie du Chien during the War of 1812"},
{name:"Guelph City Hall ",location:[43.544083,-80.247750],Date:"1857 (completed)",Designated:"1984",City:"Guelph",Type:"NHistoric",Description:"A two-storey, limestone building built in the Renaissance Revival style, it is an excellent example of a multi-functional city hall, which contained the market, fire hall, police office and jail, library, a reading room, a large public hall, along with town offices and a council chamber; symbolic of Guelph's mid-19th-century confidence following the arrival of the Grand Trunk Railway in the community"},
{name:"Hamilton and Scourge ",location:[43.2962000,-79.297944],Date:"1813 (sinking)",Designated:"1976",City:"Lake Ontario (11 kilometres (6.8Â mi) north of St. Catharines)",Type:"NHistoric",Description:"The USS Hamilton and USS Scourge were two merchant schooners pressed into service by the Americans in the War of 1812, both of which capsized and sank in a sudden squall; the ships are in remarkable condition at the underwater wreckage site and are rare examples of surviving War of 1812 vessels"},
{name:"Her Majesty's / St. Paul's Chapel of the Mohawks ",location:[43.1244472,-80.2349556],Date:"1785 (completed)",Designated:"1981",City:"Brantford",Type:"NHistoric",Description:"The first Protestant church in Upper Canada, now the oldest surviving church in Ontario, and one of only two Royal Chapels in Canada; symbolic of the important role played by the Loyalist Mohawks in the development of the province"},
{name:"Hillary House ",location:[44.0028028,-79.4686278],Date:"1862 (completed)",Designated:"1973",City:"Aurora",Type:"NHistoric",Description:"Now operating as the Koffler Museum of Medicine, an excellent example of a Picturesque house in the Gothic Revival style"},
{name:"Homer Watson House / Doon School of Fine Arts ",location:[43.3948667,-80.4184944],Date:"1834 (completed)",Designated:"1980",City:"Kitchener",Type:"NHistoric",Description:"Once the home and studio of Canadian landscape artist Homer Watson; some of Watson's most well-known works are views of the surrounding countryside from various vantage points on this property"},
{name:"Homewood ",location:[44.6332083,-75.6168694],Date:"1801 (completed)",Designated:"1982",City:"Augusta",Type:"NHistoric",Description:"A two-storey fieldstone residence built for Dr. Solomon Jones, a prominent Loyalist; the house reflects the lifestyle of a prominent rural professional in the early 19th century and its design uniquely melds the Palladian style and the rural architectural traditions of nearby Quebec"},
{name:"Huron County Gaol ",location:[43.7496750,-81.7082833],Date:"1841 (completed)",Designated:"1973",City:"Goderich",Type:"NHistoric",Description:"A distinctive octagonal jail design in the Panopticon style of prison construction"},
{name:"Inverarden House ",location:[45.0313250,-74.6710722],Date:"1823 (completed)",Designated:"1968",City:"Cornwall",Type:"NHistoric",Description:"Built for retired North West Company partner John MacDonald of Garth, the manor house is an excellent early example of Regency architecture in Canada"},
{name:"Joseph Schneider Haus ",location:[43.4448194,-80.4946278],Date:"1816 (completed)",Designated:"1999",City:"Kitchener",Type:"NHistoric",Description:"A house museum associated with the migration of German Mennonites from Lancaster County, Pennsylvania to Waterloo County in the early 19th century, and illustrative of the typical Mennonite house plan from the period"},
{name:"Kay-Nah-Chi-Wah-Nung (Rainy River Mounds) ",location:[48.646933,-94.094017],Date:"3000 BCE (ca.)",Designated:"1969",City:"Morley",Type:"NHistoric",Description:"One of the most significant centres of early habitation and ceremonial burial in Canada, with evidence of 5,000 years of human habitation, including burial mounds from the Laurel and Blackduck cultures; a cultural and historic focal point for the Rainy Lake and River Bands of Saulteaux"},
{name:"Lansdowne Iron Works ",location:[44.5493278,-76.126028],Date:"1801 (established)",Designated:"1932",City:"Leeds and the Thousand Islands",Type:"NHistoric",Description:"The first ironworks in Upper Canada, destroyed by fire after a decade in operation"},
{name:"Leaskdale Manse ",location:[44.2030972,-79.1604333],Date:"1886 (completed)",Designated:"1996",City:"Leaskdale",Type:"NHistoric",Description:"The home of Lucy Maud Montgomery when she wrote 11 of the 22 works published during her lifetime; the house and its immediate area figure prominently in her posthumously published journals"},
{name:"Leeds and Grenville County Court House ",location:[44.5908639,-75.6857472],Date:"1844 (completed)",Designated:"1966",City:"Brockville",Type:"NHistoric",Description:"A landmark building set on a hill at the top of the historic Brockville town square; one of the most grandiose district courthouses built in Upper Canada"},
{name:"Lynnwood / Campbell-Reid House ",location:[42.8378972,-80.3033222],Date:"1851 (completed)",Designated:"1972",City:"Simcoe",Type:"NHistoric",Description:"Excellent example of a modestly-sized house in the Neoclassical style; located on a rise overlooking the Lynn River"},
{name:"Macdonell-Williamson House ",location:[45.5636278,-74.3832556],Date:"1819 (completed)",Designated:"1969",City:"East Hawkesbury",Type:"NHistoric",Description:"A stone house built on the banks of the Ottawa River as a retirement home for former North West Company partner John Macdonell"},
{name:"Matheson House ",location:[44.9010889,-76.2506639],Date:"1840 (completed)",Designated:"1966",City:"Perth",Type:"NHistoric",Description:"Built for Roderick Matheson, a local merchant and politician, the house is a good example of an affluent, pre-Confederation residence; it occupies a key position in one of the best surviving historic streetscapes in Canada, and now serves as the Perth Museum"},
{name:"Mazinaw Pictographs ",location:[44.9005722,-77.2064222],Date:"nan",Designated:"1982",City:"Bon Echo Provincial Park",Type:"NHistoric",Description:"The largest rock art site on the southern Canadian Shield and the only major pictograph site in Southern Ontario"},
{name:"McCrae House ",location:[43.536028,-80.245028],Date:"1858 (ca.) (completed)",Designated:"1966",City:"Guelph",Type:"NHistoric",Description:"The birthplace of John McCrae, the author of In Flanders Fields"},
{name:"McMartin House ",location:[44.8978139,-76.2469417],Date:"1830 (completed)",Designated:"1972",City:"Perth",Type:"NHistoric",Description:"Built for Daniel McMartin, a member of Upper Canada's Tory Loyalist elite; an example of American federal architecture that was elaborate for the time and place of construction"},
{name:"Merrickville Blockhouse ",location:[44.9168167,-75.8377861],Date:"1833 (completed)",Designated:"1939",City:"MerrickvilleâWolford",Type:"NHistoric",Description:"A relatively large, British-designed blockhouse, considered an excellent example of the structures erected for the defence of the Rideau Canal in the 19th-century"},
{name:"Middleport Site ",location:[43.0983361,-80.0678278],Date:"nan",Designated:"1953",City:"Six Nations of the Grand River First Nation",Type:"NHistoric",Description:"Archaeological site related to the Middle Ontario Iroquois"},
{name:"Middlesex County Court House ",location:[42.9821083,-81.2543472],Date:"1829 (completed)",Designated:"1955",City:"London",Type:"NHistoric",Description:"A very early and nationally significant example of the Gothic Revival style in Canada; associated with the early government of the province, as the site of the building was proposed by John Graves Simcoe for the provincial capital"},
{name:"Mission of St. Ignace II ",location:[44.719389,-79.7179778],Date:"1649 (attack)",Designated:"1955",City:"Tay",Type:"NHistoric",Description:"Destruction of Saint Ignace II and nearby Saint-Louis sealed the fate of the Huron- Wendat confederacy, leading to the abandonment of their traditional homeland"},
{name:"Mnjikaning Fish Weirs ",location:[44.6043417,-79.369611],Date:"3300 BCE (ca.)",Designated:"1982",City:"Ramara",Type:"NHistoric",Description:"The site of the largest and best preserved wooden fish weirs known in eastern North America, in use from about 3300 B.C. until the recent past"},
{name:"Moose Factory Buildings ",location:[51.2786694,-80.639306],Date:"1673 (established)",Designated:"1957",City:"Moose Factory",Type:"NHistoric",Description:"19th-century buildings associated with the second Hudson's Bay Company post in Canada; after the 1821 merger with the North West Company, Moose Factory became the supply point for posts inland as far as Lake Timiskaming"},
{name:"Nanticoke ",location:[42.7976056,-80.0533389],Date:"1813 (battle)",Designated:"1924",City:"Nanticoke",Type:"NHistoric",Description:"The site where the Norfolk volunteer militia routed a band of American marauders who had been pillaging area farms and terrorizing the country, an exploit that inspired the British military forces and the people of Upper Canada during the War of 1812; now the location of the Nanticoke Generating Station"},
{name:"Napanee Town Hall ",location:[44.2485889,-76.9506889],Date:"1856 (completed)",Designated:"1984",City:"Greater Napanee",Type:"NHistoric",Description:"An early Ontario example of a combination town hall and market, and a rare extant example in Canada of a town hall in the Greek Revival style; symbolic of the development of local government in Ontario in the 19th century"},
{name:"Nazrey African Methodist Episcopal Church ",location:[42.1013583,-83.1060139],Date:"1848 (completed)",Designated:"1999",City:"Amherstburg",Type:"NHistoric",Description:"A simple fieldstone chapel, now part of the North American Black Historical Museum complex; it has an important association with Bishop Willis Nazery, the first leader of a wholly Canadian denomination (the British Methodist Episcopal Church) founded by Underground Railroad refugees"},
{name:"Normandale Furnace ",location:[42.709750,-80.309883],Date:"1818 (commenced operations)",Designated:"1927",City:"Normandale",Type:"NHistoric",Description:"The site of an early 19th-century Ontario iron smelter"},
{name:"Old Hay Bay Church ",location:[44.1031806,-77.0170361],Date:"1792 (completed)",Designated:"2001",City:"Greater Napanee",Type:"NHistoric",Description:"Built by United Empire Loyalist settlers, it is the oldest surviving Methodist building in Canada and is associated with the role played by Methodists in Upper Canadaâs early development; a significant element of the history of the United Church of Canada"},
{name:"Old Stone Church ",location:[44.4262861,-79.1158750],Date:"1853 (completed)",Designated:"1991",City:"Beaverton",Type:"NHistoric",Description:"A small rural fieldstone church; a particularly good example of the few early stone vernacular churches surviving in Canada"},
{name:"Old Stone Mill ",location:[44.6102056,-76.1223806],Date:"1810 (completed)",Designated:"1970",City:"Delta",Type:"NHistoric",Description:"A three-storey stone gristmill which played an important role in the settlement and economic development of Leeds County; one of the oldest surviving mills in Ontario"},
{name:"Old Woodstock Town Hall ",location:[43.1293806,-80.757472],Date:"1853 (completed)",Designated:"1955",City:"Woodstock",Type:"NHistoric",Description:"A two-storey, buff-brick, Italianate-style town hall building; an excellent example of a Canadian colonial adaptation of a British town hall"},
{name:"Oro African Methodist Episcopal Church ",location:[44.5026889,-79.6363528],Date:"1849 (completed)",Designated:"2000",City:"Edgar",Type:"NHistoric",Description:"A log church with an unmarked cemetery; the last built remnant of a community of Black Canadians with United Empire Loyalist roots"},
{name:"Ossossane Sites ",location:[44.6823250,-79.9512528],Date:"nan",Designated:"1982",City:"Ossossane Beach",Type:"NHistoric",Description:"Archaeological site of the principal village of Bear Clan of the Hurons"},
{name:"Our Lady of the Immaculate Conception ",location:[43.5429944,-80.2516306],Date:"1888 (completed)",Designated:"1990",City:"Guelph",Type:"NHistoric",Description:"Influenced by the medieval cathedrals of France, the church on a hill is an exceptional example of the Victorian High Gothic style in Canadian architecture"},
{name:"Oxford-on-Rideau Township Hall ",location:[44.9625972,-75.6786556],Date:"1875 (completed)",Designated:"1984",City:"Oxford Mills",Type:"NHistoric",Description:"A two-storey stone building with a cupola, this small town hall is representative of early local government in Canada"},
{name:"Park House",location:[42.103659,-83.113490],Date:"1796 (built)",Designated:"2018",City:"Amherstburg",Type:"NHistoric",Description:"Considered one of the oldest houses in the Amherstburg region, Park House is a rare example of a once-common colonial building type found in settlements and fur-trading posts across colonial North America. Through its architecture, Park House tells the story of the traders and artisans who populated the Windsor region."},
{name:"Parkhill ",location:[43.15917,-81.70778],Date:"8000 BCE (ca.)",Designated:"1982",City:"Parkhill",Type:"NHistoric",Description:"The first major Palaeo-Indian habitation site reported in Ontario, consisting of concentrations of stone artifactual material distributed over 6 hectares (15 acres)"},
{name:"Parkwood ",location:[43.9033194,-78.8684667],Date:"1940 (completed)",Designated:"1989",City:"Oshawa",Type:"NHistoric",Description:"A residential estate developed between 1915 and 1940 by Canadian industrialist Samuel McLaughlin; among the finest and most intact surviving examples of Canadian architectural and landscape design, featuring the work of Pearson and Darling, Frances Loring, John M. Lyle, Florence Wyle and others"},
{name:"Penman Textile Mill ",location:[43.1975778,-80.3896833],Date:"1874 (completed)",Designated:"1989",City:"Paris",Type:"NHistoric",Description:"The first and most important plant of the largest knitting firm in Canada, the Penman Manufacturing Company"},
{name:"Perth Town Hall ",location:[44.8991278,-76.2488833],Date:"1864 (completed)",Designated:"1984",City:"Perth",Type:"NHistoric",Description:"A two-storey stone building topped by a layered cupola with clock, symbolic of the role and importance of local government in the 19th century"},
{name:"Peterborough Drill Hall / Armoury ",location:[44.3086556,-78.3222944],Date:"1909 (completed)",Designated:"1989",City:"Peterborough",Type:"NHistoric",Description:"Built in the Romanesque Revival style, the armoury is representative of the third phase of drill hall construction in Canada (1896â1918); one of the largest and best designed examples from the 1907-1909 period"},
{name:"Peterborough Lift Lock ",location:[44.3076806,-78.3002889],Date:"1904 (completed)",Designated:"1979",City:"Peterborough",Type:"NHistoric",Description:"A large boat lift along the Trent-Severn Waterway designed to lift boats 19.8 metres (65Â ft); it is the highest hydraulic lift lock in the world, and at the time of its construction was an engineering achievement of national and international renown"},
{name:"Peterborough Petroglyphs ",location:[44.613444,-78.0441278],Date:"900 to 1400 (ca.)",Designated:"1981",City:"Otonabee-South Monaghan",Type:"NHistoric",Description:"An outcrop of exposed marble inscribed with hundreds of realistic human and animal forms, as well as numerous abstract and symbolic images; one of the largest known concentrations of pre-contact petroglyphs in Canada"},
{name:"Pic River Site ",location:[48.6263278,-86.2849056],Date:"nan",Designated:"1981",City:"Pic River",Type:"NHistoric",Description:"A site comprising four archaeological nodes: the Pic River site, Fort Pic, the Heron Bay site, and the Duncan site, which together represent numerous Aboriginal and European occupations dating from 12000 BCE to the late 19th century"},
{name:"Point Clark Lighthouse ",location:[44.0728556,-81.7573250],Date:"1859 (completed)",Designated:"1966",City:"Point Clark",Type:"NHistoric",Description:"One of six Imperial Towers on Lake Huron; noted for its distinctive lantern and the quality of its architecture"},
{name:"Pointe au Baril ",location:[44.637583,-75.609583],Date:"nan",Designated:"1923",City:"Maitland",Type:"NHistoric",Description:"The (Iroquoise) and (Outaouaise), the last two French warships that navigated Lake Ontario, were built on this point"},
{name:"Port Hope Capitol Theatre",location:[43.950798,-78.293215],Date:"1930",Designated:"2016",City:"Port Hope",Type:"NHistoric",Description:"Rare and outstanding (atmospheric )theatre of the 1920s, among the first to present (talkies)"},
{name:"Port Stanley ",location:[42.6658306,-81.2119472],Date:"1669 (first European contact)",Designated:"1923",City:"Port Stanley",Type:"NHistoric",Description:"An important landing point for a succession of explorers and travellers of the 17th and 18th centuries, marked by a cairn commemorating events including the landing of Adrien Jolliet in 1669 (during the first descent of the Great Lakes by Europeans) and the encampment by Isaac Brock and his troops in the War of 1812"},
{name:"Port Talbot ",location:[42.640050,-81.366150],Date:"1803 (established)",Designated:"1923",City:"Port Talbot",Type:"NHistoric",Description:"Founded in 1803 by Thomas Talbot, the settlement was one of the most prosperous of its time in Upper Canada, noted for its good roads, with Talbot keeping out land speculators and securing hard-working settlers; Talbot's authoritarian control of the settlers led to his downfall at the hands of colonial authorities"},
{name:"Prescott Railway Station (Grand Trunk) ",location:[44.7110167,-75.524611],Date:"1855 (completed)",Designated:"1973",City:"Prescott",Type:"NHistoric",Description:"A small, stone train station, typical of the smaller stations erected for the Grand Trunk Railway during the first construction period of the GTR line between Montreal and Brockville"},
{name:"Rideau Canal ",location:[45.42583,-75.69722],Date:"1832 (completed)",Designated:"1925",City:"Ottawa to Kingston",Type:"NHistoric",Description:"Built for the British government by Lieutenant-Colonel John By as a defensive work in the event of war with the United States, the canal is the best preserved example of a 19th-century slack water canal in North America, with most of its original structures intact"},
{name:"Ridout Street Complex ",location:[42.9836861,-81.2548500],Date:"1838 to 1870 (period of construction)",Designated:"1966",City:"London",Type:"NHistoric",Description:"Comprising three mid-19th-century residential and commercial buildings, the grouping is representative of the appearance of Ontario cities in that period and of London's early residential and commercial architecture"},
{name:"Rosamond Woollen Mill ",location:[45.2281028,-76.2003667],Date:"1866 (established)",Designated:"1986",City:"Almonte",Type:"NHistoric",Description:"At one time one of the largest mills in Canada, it is characteristic of late 19th-century textile mills in Canada"},
{name:"Royal Flying Corps Hangars ",location:[44.2689278,-79.8884583],Date:"1917 (completed)",Designated:"1989",City:"Essa",Type:"NHistoric",Description:"Eight surviving First World War hangars at CFB Borden which played a significant role in the development of military aviation in Canada"},
{name:"Ruin of St. Raphael's Roman Catholic Church ",location:[45.2117194,-74.5970389],Date:"1821 (main church completed)",Designated:"1996",City:"South Glengarry",Type:"NHistoric",Description:"St. Raphael's roof, tower and interior decoration were destroyed by fire in 1970, but the ruins were preserved; one of the earliest surviving Roman Catholic monuments in English-speaking Canada"},
{name:"Ruthven Park ",location:[42.979278,-79.8750889],Date:"1846 (completed)",Designated:"1995",City:"Cayuga",Type:"NHistoric",Description:"An estate with a Greek Revival villa, laid out by David Thompson; associated with the transformation of Upper Canada from a (settler) to a (settled) society"},
{name:"Saint-Louis Mission ",location:[44.7290972,-79.7812833],Date:"1649 (attack)",Designated:"1920",City:"Victoria Harbour",Type:"NHistoric",Description:"A 2-hectare (4.9-acre) archaeological site, once the site of a stockaded Huron-Wendat village and nearby Jesuit mission; an Iroquois attack in 1649 led to a chain of events resulting in the abandonment of Huronia by the Huron-Wendat in 1650"},
{name:"Sainte-Marie Among the Hurons Mission ",location:[44.7343639,-79.8454194],Date:"1639 (established)",Designated:"1920",City:"Midland",Type:"NHistoric",Description:"The reconstructed main Jesuit mission to the Huron-Wendat; over ten years, it grew into a sizeable colony, but was abandoned in 1649 due to disease and conflict"},
{name:"Sandwich First Baptist Church ",location:[42.2917972,-83.0802556],Date:"1851 (completed)",Designated:"1999",City:"Windsor",Type:"NHistoric",Description:"One of the oldest Baptist churches surviving from this period in Ontario; representative of the churches in border settlements built to accommodate the communities created by Underground Railroad refugees"},
{name:"Sault Ste. Marie Canal ",location:[46.51278,-84.35139],Date:"1895 (completed)",Designated:"1987",City:"Sault Ste. Marie",Type:"NHistoric",Description:"Now operated as a recreational facility, the canal and associated works were a significant part of Canada's national canal system at the time of their construction; the first electrically-powered lock in the world"},
{name:"Serpent Mounds ",location:[44.21111,-78.15694],Date:"50 BCE to 300 CE (ca.)",Designated:"1982",City:"Keene",Type:"NHistoric",Description:"A grouping of six separate burial mounds forming a serpentine shape approximately 60 metres (200Â ft) long; the only one of its kind in Canada"},
{name:"Sharon Temple ",location:[44.101333,-79.4418750],Date:"1831 (completed)",Designated:"1990",City:"Sharon",Type:"NHistoric",Description:"A temple built by the Children of Peace, a schismatic Quaker sect, and its design and aesthetics embody the values of the group; acquired by a local historic group in 1917, and is representative of the early heritage conservation movement in Canada"},
{name:"Sheguiandah ",location:[45.8779694,-81.9011444],Date:"11,000 BCE (ca.) (first occupation)",Designated:"1954",City:"Manitoulin District",Type:"NHistoric",Description:"An archaeological site on Manitoulin Island where successive aboriginal peoples quarried quartzite, leaving behind artifacts spanning approximately 10,000 years of occupation"},
{name:"Sir John Johnson House ",location:[45.1447361,-74.5794750],Date:"1792 (original core completed)",Designated:"1961",City:"Williamstown",Type:"NHistoric",Description:"A wooden house of typical 19th-century Ontario vernacular design; associated with noted Loyalist Sir John Johnson, 2nd Baronet"},
{name:"Smiths Falls Bascule Bridge ",location:[44.8972139,-76.0270528],Date:"1913 (completed)",Designated:"1983",City:"Smiths Falls",Type:"NHistoric",Description:"A bascule bridge over the Rideau Canal; an early example of distinctive design in movable bridges, it is the oldest surviving structure of its type"},
{name:"Smiths Falls Railway Station (Canadian Northern) ",location:[44.8991889,-76.0277083],Date:"1914 (completed)",Designated:"1983",City:"Smiths Falls",Type:"NHistoric",Description:"Now serving as the Smiths Falls Railway Museum, the station's distinctive turret, polygonal waiting room and substantial construction were a departure from the usual practice of building cheaply from standard plans; representative of Canadian Northern Railway's efforts to compete directly with the Canadian Pacific Railway"},
{name:"Southwold Earthworks ",location:[42.674317,-81.36059067],Date:"1500 (ca.)(occupation)",Designated:"1923",City:"Iona",Type:"NHistoric",Description:"Rare and well-preserved archaeological remains of an Attawandaron fortified village"},
{name:"St. Jude's Anglican Church ",location:[43.1398722,-80.2534917],Date:"1871 (completed)",Designated:"1993",City:"Brantford",Type:"NHistoric",Description:"A small 1871 church in the High Victorian Gothic Revival style; known for its striking interior murals, painted in 1936, influenced by the Arts and Crafts Movement and the work of William Morris"},
{name:"St. Marys Junction Railway Station (Grand Trunk) ",location:[43.2718222,-81.1312583],Date:"1858 (completed)",Designated:"1973",City:"St. Marys Junction",Type:"NHistoric",Description:"A one-storey limestone building in the Italianate style which served for a time as the western terminus of the Grand Trunk Railway; representative of the Grand Trunk's original Ontario stations, and a rare surviving stone example"},
{name:"St. Thomas City Hall ",location:[42.7790972,-81.192722],Date:"1899 (completed)",Designated:"1984",City:"St. Thomas",Type:"NHistoric",Description:"A well-designed late Victorian civic building with a commanding clock tower, reflective of the tremendous growth of the city at the end of the 19th century; one of the few surviving unaltered Richardsonian Romanesque town halls in Canada"},
{name:"Stephen Leacock Museum / Old Brewery Bay ",location:[44.6082583,-79.3939778],Date:"1928 (completed)",Designated:"1992",City:"Orillia",Type:"NHistoric",Description:"A two-storey house on a four 4-hectare (9.9-acre) property on the shores of Lake Couchiching; home of and inspiration to Stephen Leacock, one of Canada's most celebrated authors and humorists"},
{name:"Stratford City Hall ",location:[43.3699972,-80.9821889],Date:"1899 (completed)",Designated:"1976",City:"Stratford",Type:"NHistoric",Description:"A monumental red brick city hall with a prominent clock tower; a noted late 19th-century Picturesque municipal building, representative of an era when growing cities sought to express their civic pride and ambition in impressive civic buildings"},
{name:"Thistle Ha' Farm ",location:[43.9397333,-79.109139],Date:"1852 (livestock operation began)",Designated:"1973",City:"Claremont",Type:"NHistoric",Description:"An 80-hectare (200-acre) working farm where groundbreaking breeding of pedigree livestock in Canada occurred in the 19th century; the work undertaken here played an important role in improving stock breeding throughout the Americas"},
{name:"Thunder Bay Tourist Pagoda ",location:[48.4345722,-89.2180500],Date:"1909 (completed)",Designated:"1986",City:"Thunder Bay",Type:"NHistoric",Description:"An octagonal brick tourism bureau featuring a pagoda-shaped roof with cupola, surrounded by a verandah and with an entrance surmounted by a carved beaver; symbolic of civic boosterism and novelty architecture"},
{name:"Trent to Severn Waterway ",location:[44.1208250,-77.5914194 ],Date:"1830 (commenced) - 1920 (completed)",Designated:"1929",City:"Trenton",Type:"NHistoric",Description:"A 386-kilometre (240Â mi) natural and man-made waterway that links Georgian Bay to the Bay of Quinte; an important transportation route noted for the high number of surviving unmodified structures in some sections dating from the waterway's construction"},
{name:"Severn Waterway",location:[44.8037000,-79.7207028],Date:"1830 (commenced) - 1920 (completed)",Designated:"1929",City:"Cobourg",Type:"NHistoric",Description:"A 386-kilometre (240Â mi) natural and man-made waterway that links Georgian Bay to the Bay of Quinte; an important transportation route noted for the high number of surviving unmodified structures in some sections dating from the waterway's construction"},
{name:"Victoria Hall / Cobourg Town Hall ",location:[43.9594111,-78.1678083],Date:"1860 (completed)",Designated:"1959",City:"Petrolia",Type:"NHistoric",Description:"A neoclassical three-storey civic building; one of the more extravagant examples of town halls constructed in Canada West after landmark municipal legislation in 1848, symbolic of the prosperity and optimism of Cobourg during the 1850s"},
{name:"Victoria Hall / Petrolia Town Hall ",location:[42.8811972,-82.1468250],Date:"1889 (completed)",Designated:"1975",City:"Onondaga",Type:"NHistoric",Description:"A buff brick town hall with prominent clock tower, reflective of the time in the 1880s when Petrolia was among the wealthiest towns in Canada due to the local oil boom"},
{name:"Walker Site ",location:[43.11667,-80.11667],Date:"nan",Designated:"1982",City:"Fergus",Type:"NHistoric",Description:"Large Iroquoian archaeological site pertaining to the historic Attiwandaronk tribe"},
{name:"Wellington County House of Industry and Refuge ",location:[43.6930722,-80.3997722],Date:"1877 (completed)",Designated:"1995",City:"Sault Ste. Marie",Type:"NHistoric",Description:"A former farm dominated by a two-storey Italianate-style stone building; oldest known state-supported poorhouse or almshouse in Canada, now serving as the county museum and archives"},
{name:"Whitefish Island ",location:[46.511000,-84.354000],Date:"300 BCE (ca.) (first Aboriginal encampments)",Designated:"1981",City:"Wilberforce",Type:"NHistoric",Description:"A small, naturalized island that was occupied by eight successive cultures, culminating in the development of the Ojibwa nation; its location made it a focal point of pre-contact trade and settlement"},
{name:"Wilberforce Red Cross Outpost ",location:[45.0389056,-78.2235889],Date:"1916 (completed); 1922-57 (health centre operational)",Designated:"2003",City:"Port Dover",Type:"NHistoric",Description:"A simple frame house that served as the first outpost of the Ontario Division of the Canadian Red Cross; dedicated women provided nurse care with minimal medical backup, facilities and equipment, and the site is representative of the key role of nurses in providing health care and education in isolated areas"},
{name:"Wintering Site ",location:[42.7946250,-80.189333],Date:"1669â70 (event)",Designated:"1919",City:"Wolfe Island",Type:"NHistoric",Description:"Explorers FranÃ§ois Dollier de Casson and RenÃ© de BrÃ©hant de GalinÃ©e and seven other Frenchmen wintered at this site - the first Europeans known to have ascended the Great Lakes to Sault Ste. Marie"},
{name:"Wolfe Island Township Hall ",location:[44.1931222,-76.4408000],Date:"1859 (completed)",Designated:"1984",City:"London",Type:"NHistoric",Description:"A small Italianate rural town hall; an unusually sophisticated example of its type"},
{name:"Wolseley Barracks ",location:[43.000139,-81.2338972],Date:"1888 (completed)",Designated:"1963",City:"Kitchener",Type:"NHistoric",Description:"The first purpose-built infantry training school erected by the federal government, originally used to house Company (D) of the Infantry School Corps of The Royal Canadian Regiment; an important early step in the development of a permanent military force in Canada following the withdrawal of regular British troops from Canada in 1871"},
{name:"Woodside ",location:[43.4635778,-80.480722],Date:"1853 (completed)",Designated:"1952",City:"nan",Type:"NHistoric",Description:"A one-and-a-half-storey house on a wooded estate which served as the childhood home of William Lyon Mackenzie King, the longest serving Prime Minister of Canada; the subject of many of King's writings in adult life"},
{name:"Battle of Stoney Creek ",location:[43.217271, -79.7662],Date:"1813 (battle)",Designated:"1960",City:"Stoney Creek",Type:"PHistoric",Description:"The site of a British victory that marked a turning point in the War of 1812, representing the most advanced position achieved by American forces in the Niagara campaign"},
{name:"Burlington Heights ",location:[43.27056, -79.88611],Date:"1813-14 (wartime activities)",Designated:"1929",City:"Hamilton",Type:"PHistoric",Description:"An assembly point and supply depot for the defence of the Niagara Peninsula and support of the navy on Lake Ontario during the War of 1812"},
{name:"Dundurn Castle ",location:[43.269481, -79.884649],Date:"1835 (completed)",Designated:"1984",City:"Hamilton",Type:"PHistoric",Description:"Picturesque-style villa of magnate Sir Allan Napier MacNab, 1st Baronet"},
{name:"Erland Lee (Museum) Home ",location:[43.20667, -79.72167],Date:"1808 (completed)",Designated:"2002",City:"Hamilton",Type:"PHistoric",Description:"A Carpenter Gothic farmhouse recognized as the birthplace of an important national and international women's movement, where the constitution of the first Women's Institute was drafted"},
{name:"Former Hamilton Customs House ",location:[43.2663639, -79.8672139],Date:"1860 (completed)",Designated:"1990",City:"Hamilton",Type:"PHistoric",Description:"A former customs house now serving as the Workers Arts and Heritage Centre; a noted example of Italianate architecture, which was popular in Canada from the 1840s to the 1870s; based on designs by Frederick Preston Rubidge 1858"},
{name:"Former Hamilton Railway Station (Canadian National) ",location:[43.2663639, -79.8672139],Date:"1931 (completed)",Designated:"2000",City:"Hamilton",Type:"PHistoric",Description:"Built by Canadian National Railway, the railway station is a rare surviving example of an interwar station built according to the tenets of the City Beautiful movement; it served as an important immigration gateway after the Second World War"},
{name:"Griffin House ",location:[43.2359500, -80.0031278],Date:"1827 (completed)",Designated:"2008",City:"Hamilton",Type:"PHistoric",Description:"A rare surviving example of a four-room house typical in Upper Canada in the early 19th century; was owned by Enerals Griffin, a Black immigrant from Virginia who settled here in 1834, and the house is associated with Black settlement in British North America and the Underground Railroad"},
{name:"Hamilton Waterworks ",location:[43.2562361, -79.7706972],Date:"1859 (completed)",Designated:"1977",City:"Hamilton",Type:"PHistoric",Description:"Built to deliver large quantities of clean water for safe drinking and fire control to rapidly expanding Hamilton, the waterworks is a rare surviving example of a Victorian industrial complex that is largely architecturally and functionally intact"},
{name:"HMCS Haida ",location:[43.27531, -79.85538],Date:"1942 (constructed)",Designated:"1984",City:"Hamilton",Type:"PHistoric",Description:"Last of the World War II Tribal-class destroyers; moored and open to the public as a museum ship at Hamilton Harbour"},
{name:"John Weir Foote Armoury ",location:[43.2618778, -79.8662278],Date:"1888 (completed)",Designated:"1989",City:"Hamilton",Type:"PHistoric",Description:"Named after John Weir Foote, the north section of the building is representative of the second evolutionary stage in drill hall construction in Canada (in the 1870s to 1890s)"},
{name:"McQuesten House / Whitehern ",location:[43.2546, -79.8721],Date:"1848 (completed)",Designated:"1962",City:"Hamilton",Type:"PHistoric",Description:"The two-storey neoclassical home of Thomas McQuesten, now serving as a museum; a superior and intact example of mid-19th-century residential architecture in Ontario"},
{name:"Royal Botanical Gardens ",location:[43.2909833, -79.8751972],Date:"1920s (established)",Designated:"1993",City:"Hamilton",Type:"PHistoric",Description:"Comprising 1,100 hectares (2,700 acres) clustered around Burlington Bay, it is one of Canada's most important botanical gardens, and is the international registration authority for cultivar names of lilacs; named Canada's (National Focal Point) for plant conservation targets under the United Nations Convention on Biological Diversity"},
{name:"Sandyford Place ",location:[43.2519389, -79.8732556],Date:"1856 (completed)",Designated:"1975",City:"Hamilton",Type:"PHistoric",Description:"A row of stone terrace houses, typical of the construction style in Hamilton at a time when Scottish settlers sought to recreate the stone terraces of Scottish towns; a good example of the housing erected for merchants in the mid-19th century"},
{name:"St. Paul's Presbyterian Church / Former St. Andrew's Church ",location:[43.254761, -79.870282],Date:"1857 (completed)",Designated:"1990",City:"Hamilton",Type:"PHistoric",Description:"An excellent representative example of the Gothic Revival style in a small, urban parish church"},
{name:"Victoria Hall ",location:[43.255691, -79.867267],Date:"1888 (completed)",Designated:"1995",City:"Hamilton",Type:"PHistoric",Description:"A three-and-a-half-storey, commercial building with a hand-made, galvanized sheet-metal faÃ§ade on the front of its upper storeys; a very rare example of an in-situ, hand-made, sheet-metal faÃ§ade in Canada, and one of the most architecturally accomplished of the surviving sheet metal faÃ§ades in the country"},
{name:"Ann Baillie Building ",location:[44.2237667, -76.4923694],Date:"1904 (completed)",Designated:"1997",City:"Kingston ",Type:"PHistoric",Description:"One of the first purpose-built nurses residences in Canada, the building represents the professionalization of nursing in Canada in the early 20th-century, and now serves as the Museum of Health Care"},
{name:"Bellevue House ",location:[44.22278, -76.50333],Date:"1841 (completed)",Designated:"1995",City:"Kingston",Type:"PHistoric",Description:"A noted example of Italianate architecture in the Picturesque manner in Canada, and the former residence of John A. Macdonald, a Father of Confederation and the first Prime Minister of Canada"},
{name:"Cataraqui Cemetery ",location:[44.264, -76.54111],Date:"1850 (established)",Designated:"2011",City:"Kingston",Type:"PHistoric",Description:"One of the best examples of a medium-sized rural or garden cemetery in Canada, containing a range of remarkable monuments, a Gothic Revival lodge, and the graves of many notable Canadians, including John A. Macdonald, the first Prime Minister (itself a NHS)"},
{name:"Elizabeth Cottage ",location:[44.23179, -76.4889611],Date:"1843 (completed)",Designated:"1993",City:"Kingston",Type:"PHistoric",Description:"A representative example of a 19th-century Gothic Revival villa"},
{name:"Fort Frontenac ",location:[44.23333, -76.47861],Date:"1673 (original fort completed)",Designated:"1923",City:"Kingston",Type:"PHistoric",Description:"Originally a French trading post that served as a gateway to the West, the base of Robert de LaSalles explorations and a French outpost against the Iroquois and English forces"},
{name:"Fort Henry ",location:[44.2302639, -76.4596806],Date:"1840 (completed)",Designated:"1923",City:"Kingston",Type:"PHistoric",Description:"British fort that served as the principal fortification among a series of military works designed to defend Kingston, its harbour and dockyard and the entrance to the Rideau Canal"},
{name:"Frontenac County Court House ",location:[44.227777, -76.489777],Date:"1858 (completed)",Designated:"1980",City:"Kingston",Type:"PHistoric",Description:"Representative of the large-scale court houses erected in Ontario after 1850, when the Municipal Act was amended to give increased power to counties to construct court houses on a monumental scale to accommodate various county functions"},
{name:"Kingston City Hall and Market Square",location:[44.2299111, -76.480583],Date:"1844 (completed)",Designated:"1961",City:"Kingston",Type:"PHistoric",Description:"A prominent example of the Neoclassical style in Canada, with a landmark tholobate and dome; its scale and design are reflective of Kingtson's status at the time of construction as capital of the Province of Canada . The Kingston Public Market, founded in 1801, is behind city hall and part of the national historic site and is the oldest public market in Ontario."},
{name:"Kingston Customs House ",location:[44.2296222, -76.4823472],Date:"1859 (completed)",Designated:"1971",City:"Kingston",Type:"PHistoric",Description:"A limestone former customs house; an excellent example of the architectural quality of mid-19th-century public buildings designed in the British classical tradition"},
{name:"Kingston Dry Dock ",location:[44.2251750, -76.4833778],Date:"1892 (completed)",Designated:"1978",City:"Kingston",Type:"PHistoric",Description:"An important construction and repair facility for ships on the Great Lakes; noted for the Second World War naval vessels, notably corvettes, built in this dry dock"},
{name:"Kingston Fortifications ",location:[44.222275, -76.490357],Date:"1840 (completed)",Designated:"1989",City:"Kingston",Type:"PHistoric",Description:"A fortification system consisting of five installations (Fort Henry NHS, Fort Frederick, Murney Tower NHS, Shoal Tower NHS and Cathcart Tower), crucial to the 19th century defense of Kingston and the terminus of the Rideau Canal"},
{name:"Kingston General Hospital ",location:[44.22417, -76.49306],Date:"1833-1924 (completion of historic buildings)",Designated:"1995",City:"Kingston",Type:"PHistoric",Description:"A complex of limestone buildings, built between 1833 and 1924, set in a campus of more recent hospital buildings; the oldest public hospital in Canada still in operation, with facilities illustrative of health care in Canada in the 19th and 20th centuries"},
{name:"Kingston Navy Yard ",location:[44.22889, -76.46861],Date:"1788 (established)",Designated:"1928",City:"Kingston",Type:"PHistoric",Description:"The site of a Royal Navy Dockyard from 1788 to 1853"},
{name:"Kingston Penitentiary ",location:[44.22069, -76.51340],Date:"1835 (established)",Designated:"1990",City:"Kingston",Type:"PHistoric",Description:"Canada's oldest reformatory prison, with a layout that served as a model for other federal prisons for more than a century; its massive stone wall and north gate are an imposing local landmark"},
{name:"Murney Tower",location:[44.22228, -76.490582],Date:"1846 (completed)",Designated:"1930",City:"Kingston",Type:"PHistoric",Description:"A martello tower located on Murray Point on the west shore of Kingston Harbour; also a component of the Kingston Fortifications National Historic Site of Canada"},
{name:"Old Kingston Post Office ",location:[44.2299750, -76.4830250],Date:"1859 (completed)",Designated:"1971",City:"Kingston",Type:"PHistoric",Description:"A two-storey, limestone building built in the Neoclassical style, illustrative of the popularity of neoclassical elements in the mid-19th century and the eclecticism of early Victorian architecture in Canada"},
{name:"Point Frederick Buildings ",location:[44.2279806, -76.4694778],Date:"1790 to 1846 (completed)",Designated:"1973",City:"Kingston",Type:"PHistoric",Description:"A peninsula upon which a major British naval base was located during the War of 1812; an assemblage of architecturally significant structures used by the Royal Military College of Canada"},
{name:"Rideau Canal ",location:[45.42583, -75.69722],Date:"1837 (completed)",Designated:"1925",City:"Ottawa to Kingston",Type:"PHistoric",Description:"Built for the British government by Lieutenant-Colonel John By as a defensive work in the event of war with the United States, the canal is the best preserved example of a 19th-century slack water canal in North America, with most of its original structures intact"},
{name:"Roselawn ",location:[44.2255500, -76.5101833],Date:"1841 (completed)",Designated:"1969",City:"Kingston",Type:"PHistoric",Description:"A two-storey neoclassical house, now used as a conference centre by Queen's University; at one time the centre of a large estate, it is representative of the large 19th-century country houses built for affluent Kingstonians just beyond the (then) city outskirts"},
{name:"Shoal Tower ",location:[44.2288028, -76.47809],Date:"1847 (completed)",Designated:"1930",City:"Kingston",Type:"PHistoric",Description:"A martello tower located on a shoal in Kingston harbour; a component of the Kingston Fortifications NHS, and symbolic of Kingston's military and naval significance in the 19th century"},
{name:"Sir John A. Macdonald Gravesite ",location:[44.262080, -76.542188],Date:"1891 (burial)",Designated:"1938",City:"Kingston",Type:"PHistoric",Description:"The burial place of Sir John A. Macdonald, a Father of Confederation and the first Prime Minister of Canada, in Cataraqui Cemetery NHS"},
{name:"War of 1812 Shipwrecks ",location:[44.231003, -76.452241],Date:"1814 (built)",Designated:"2015",City:"Kingston",Type:"PHistoric",Description:"Wrecks of the British ships Saint Lawrence, Princess Charlotte, and Prince Regent in Deadman Bay and elsewhere"},
{name:"Battle of Beaver Dams ",location:[43.117776, -79.185419],Date:"1813 (battle)",Designated:"1921",City:"Thorold",Type:"PHistoric",Description:"The site of a decisive British victory during the War of 1812, after heroine Laura Secord had earlier struck out on a long and difficult trek to warn the British of an impending American attack"},
{name:"Battle of Chippawa ",location:[43.052127, -79.024720],Date:"1814 (battle)",Designated:"1921",City:"Chippawa",Type:"PHistoric",Description:"The site of a battle that took place during the War of 1812 as a result of the last major American invasion of Canada"},
{name:"Battle of Cook's Mills ",location:[42.997895, -79.174913],Date:"1814 (battle)",Designated:"1921",City:"Cooks Mills",Type:"PHistoric",Description:"The site of the last engagement between U.S. and British/Canadian armies fought in Niagara, and second-last on Canadian soil during the War of 1812"},
{name:"Battle of Lundy's Lane ",location:[43.089152, -79.095456],Date:"1814 (battle)",Designated:"1937",City:"Niagara Falls",Type:"PHistoric",Description:"The site of a spontaneous confrontation between the British and American forces in which the British attacked American forces returning from the Battle of Chippawa; the six-hour-long battle was one of the bloodiest battles of the War of 1812 and marked the end of American offensive in Upper Canada"},
{name:"Battlefield of Fort George ",location:[43.262033, -79.084517],Date:"1813 (battle)",Designated:"1921",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"The site of one of the fiercest battles of the War of 1812, in which the U.S. managed to gain a toehold on the Niagara Peninsula; distinct from nearby Fort George National Historic Site"},
{name:"Butler's Barracks ",location:[43.248370, -79.0740],Date:"1814 to 54 (completed)",Designated:"1963",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"A complex of five wooden buildings, built by the British after the War of 1812 and occupied as a military camp until the 1960s, representing 150 years of military history"},
{name:"Former L.J. Shickluna Service Station ",location:[42.887941, -79.251214],Date:"1924 (c.) (completed)",Designated:"1995",City:"Port Colborne",Type:"PHistoric",Description:"Early Spanish Colonial Revival-style service station; symbol of rapid post-World War I automobile culture"},
{name:"Fort Drummond ",location:[43.160147, -79.052234],Date:"1814 (completed)",Designated:"1928",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"A redoubt and battery constructed during the War of 1812 to protect the main portage road from Chippawa to Queenston, named after Sir Gordon Drummond; some walls of the redoubt are still extant"},
{name:"Fort Erie ",location:[42.8932500, -78.9238167],Date:"1808 (completion of third Fort Erie)",Designated:"1933",City:"Fort Erie (town)",Type:"PHistoric",Description:"The first British fort to be constructed as part of a network developed after the Seven Years' War in the western portion of the Province of Quebec (later Upper Canada); captured and destroyed in 1814 by invading American forces during the War of 1812, the remains of the fort were rebuilt by the Niagara Parks Commission in 1937 to 39"},
{name:"Fort George ",location:[43.25083, -79.06111],Date:"1799 (completed), 1940 (reconstruction completed)",Designated:"1921",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"Reconstructed British fort that served as the principal fortification on the Niagara Peninsula during the War of 1812; distinct from the nearby Battlefield of Fort George National Historic Site"},
{name:"Fort Mississauga ",location:[43.26167, -79.07667],Date:"1814 (completed)",Designated:"1960",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"Located in a strategic position at the mouth of the Niagara River to protect the Canadian frontier and to counter Fort Niagara on the U.S. side, it is the only remaining fortification of its type (a square tower within a star-shaped earthwork) in Canada"},
{name:"Frenchman's Creek ",location:[42.94227, -78.926],Date:"1812 (battle)",Designated:"1921",City:"Fort Erie",Type:"PHistoric",Description:"The battle of Frenchmans Creek was a minor skirmish in the War of 1812, where British forces pushed an American advance parties back across the Niagara River; the failure of American troops contributed, in part, to the cancellation of the larger invasion planned for the Niagara peninsula at the end of 1812"},
{name:"Mississauga Point Lighthouse ",location:[43.26164, -79.0766278],Date:"1804 (completed), 1814 (demolished)",Designated:"1937",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"The first lighthouse on the Great Lakes, constructed in 1804 by the military masons of the 49th Regiment of Foot, it was damaged in the Battle of Fort George in 1813, and demolished in 1814 when the British built Fort Mississauga on the site"},
{name:"Navy Island ",location:[43.0564083, -79.010583],Date:"1761 (shipyard established)",Designated:"1921",City:"Niagara Falls",Type:"PHistoric",Description:"Archaeological remains on an uninhabited island on the Ontario side of the Niagara River; during the 1760s, the island was home to the first British shipyard to serve the Upper Great Lakes and, during the Upper Canada Rebellion, the seat of William Lyon Mackenzies exiled government, the Republic of Canada"},
{name:"Niagara Apothecary ",location:[43.2550694, -79.070917],Date:"1820 (completed)",Designated:"1968",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"A white, single-storey clapboard Georgian building on Niagara-on-the-Lakes main commercial street, it served as an apothecary/pharmacy from approximately 1866 to 1964 and it is one of the very few remaining examples of an old apothecary shop"},
{name:"Niagara District Court House ",location:[43.2550694, -79.070917],Date:"1847 (completed)",Designated:"1980",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"Designed by William Thomas in the Neoclassical style, it is an excellent example of a mid-19th-century multipurpose civic structure, originally containing the local court house, jail, market and town hall"},
{name:"Niagara-on-the-Lake ",location:[43.2550056, -79.071833],Date:"1815 to 59 (completed)",Designated:"2003",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"An early-19th-century Loyalist town located on the southern shore of Lake Ontario; the historic district covers 25 city blocks and contains the best collection of buildings in Canada from the period following the War of 1812"},
{name:"Point Abino Light Tower ",location:[42.8355139, -79.0951778],Date:"1918 (completed)",Designated:"1998",City:"Crystal Beach",Type:"PHistoric",Description:"A lighthouse situated at the eastern end of Lake Erie; intended to complement the summer homes nearby, it was more elaborately designed than most Canadian lighthouses, with a high level of its features still intact"},
{name:"Queenston Heights ",location:[43.160147, -79.052234],Date:"1812 (battle)",Designated:"1968",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"A treed promontory on the Niagara Escarpment, where the British repulsed an American invasion in the Battle of Queenston Heights in the War of 1812; site of Brock's Monument and a monument to Laura Secord"},
{name:"Queenston-Chippawa Hydro-electric Plant ",location:[43.1459639, -79.0454306],Date:"1925 (completed)",Designated:"1990",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"At the time of its completion, it was the first large-scale hydroelectric generation project in the world"},
{name:"R. Nathaniel Dett British Methodist Episcopal Church ",location:[43.087806, -79.0874861],Date:"1836 (completed)",Designated:"1999",City:"Niagara Falls",Type:"PHistoric",Description:"A British Methodist Episcopal Church chapel named after and associated with the formative years of composer Robert Nathaniel Dett; representative of early black settlement in the Niagara area and the church's role in assisting Underground Railroad refugees"},
{name:"Ridgeway Battlefield ",location:[42.904349, -79.0416],Date:"1866 (battle)",Designated:"1921",City:"Fort Erie",Type:"PHistoric",Description:"The site of the battlefield where the Queens Own Rifles, 13th Hamilton Battalion, and Caledonia and York Rifle Companies of Haldimand defended Canada against Fenian raiders from the United States"},
{name:"Salem Chapel, British Methodist Episcopal Church ",location:[43.1651972, -79.24004],Date:"1855 (completed)",Designated:"1999",City:"St. Catharines",Type:"PHistoric",Description:"A gable-fronted church representative of the auditory-hall design of Underground Railroad-related churches; an important centre of 19th-century abolitionist activity in Canada, associated with Harriet Tubman"},
{name:"Toronto Power Generating Station ",location:[43.0720694, -79.0738167],Date:"1913 (completed)",Designated:"1983",City:"Niagara Falls",Type:"PHistoric",Description:"A generating station to supply hydro-electric power to Toronto, it was the first wholly Canadian-owned hydro-electric facility at Niagara Falls; an elegant and unusual application of Beaux-Arts design to an industrial site in Canada"},
{name:"Willowbank ",location:[43.16773, -79.05796],Date:"1836 (completed)",Designated:"2003",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"A 5-hectare (12-acre) estate with a large three-and-a-half storey temple-fronted mansion; the landscaping and largely unchanged exterior architecture are symbolic of the fusion of Neoclassical architecture and the Picturesque ideal that characterized large country estates of Upper Canada in the early 19th century"},
{name:"Vrooman's Battery ",location:[43.1782667, -79.0583472],Date:"1812 (battle)",Designated:"1921",City:"Niagara-on-the-Lake",Type:"PHistoric",Description:"An artillery battery that maintained a harassing fire on the American forces crossing the Niagara River during the Battle of Queenston Heights in the War of 1812; now consists of a mound on the riverbank"},
{name:"Aberdeen Pavilion ",location:[45.4000417, -75.6828222],Date:"1898 (completed)",Designated:"1983",City:"Ottawa",Type:"PHistoric",Description:"The only large-scale exhibition building in Canada surviving from the 19th century, and also the oldest surviving venue in which the Stanley Cup was contested"},
{name:"Beechwood Cemetery ",location:[45.4469, -75.6599],Date:"1873 (established)",Designated:"2000",City:"Ottawa",Type:"PHistoric",Description:"An exceptional example of 19th-century cemetery design, containing a concentration of mausolea, monuments, and markers of significant importance to the history of Canada, Ontario and Ottawa; the cemetery was declared the national cemetery of Canada in 2009. It has served as the national military cemetery since 1944 and the RCMP's national memorial cemetery since 2004."},
{name:"Billings House ",location:[45.389832, -75.672235],Date:"1829 (completed)",Designated:"1968",City:"Ottawa",Type:"PHistoric",Description:"One of Ottawa's oldest homes, this Georgian homestead was built by Braddish Billings, the first settler of Gloucester Township, where his home formed the nucleus of Ottawa's Billings Bridge area"},
{name:"Central Chambers ",location:[45.4232361, -75.6952639],Date:"1891 (completed)",Designated:"1990",City:"Ottawa",Type:"PHistoric",Description:"A noted example of Queen Anne Revival commercial architecture, with a high profile location on Confederation Square"},
{name:"Central Experimental Farm ",location:[45.382537, -75.713654],Date:"1886 (established)",Designated:"1997",City:"Ottawa",Type:"PHistoric",Description:"A rare example of a farm within a city; the site of significant scientific contributions to agriculture"},
{name:"ChÃ¢teau Laurier ",location:[45.4255667, -75.6951083],Date:"1912 (completion of first wing)",Designated:"1980",City:"Ottawa",Type:"PHistoric",Description:"One of Canada's landmark railway hotels, built in the distinctly Canadian ChÃ¢teau-style; dubbed the (Third Chamber of Parliament) due to its proximity to Parliament Hill"},
{name:"Confederation Square ",location:[45.4240889, -75.695500],Date:"1939 (established)",Designated:"1984",City:"Ottawa",Type:"PHistoric",Description:"The second most important ceremonial centre in Ottawa, after Parliament Hill, with Canada's National War Memorial at its centre and the Valiants Memorial at its periphery, and framed by the ChÃ¢teau Laurier, the Government Conference Centre, the National Arts Centre, the Central Chambers, the Scottish-Ontario Chambers, the Central Post Office, the Langevin Block and the East Block"},
{name:"Connaught Building ",location:[45.426562, -75.694669],Date:"1916 (completed)",Designated:"1990",City:"Ottawa",Type:"PHistoric",Description:"A testament to Wilfrid Lauriers commitment to enhance the architecture in Canada's capital; one of the best works of David Ewart, the Chief Dominion Architect from 1896 to 1914"},
{name:"Diefenbunker / Central Emergency Government Headquarters ",location:[45.35167, -76.04722],Date:"1959 (completed)",Designated:"1994",City:"Ottawa",Type:"PHistoric",Description:"An underground 4-storey bunker, capable of withstanding a near-hit from a nuclear explosion, built to shelter key Canadian political and military personnel in the event of a nuclear war, now a museum"},
{name:"Earnscliffe ",location:[45.437378, -75.698912],Date:"1857 (completed)",Designated:"1960",City:"Ottawa",Type:"PHistoric",Description:"A house overlooking the Ottawa River, once the home to Canada's first Prime Minister, Sir John A. Macdonald, now the official residence of the British High Commissioner to Canada"},
{name:"Former Archives Building ",location:[45.43056, -75.6988583],Date:"1906 (completed)",Designated:"1990",City:"Ottawa",Type:"PHistoric",Description:"The home of the Public Archives of Canada from 1906 to 1967, and the Canadian War Museum from 1967 to 2005, this building was constructed as part of Prime Minister Sir Wilfrid Lauriers efforts to transform Ottawa from a lumber town into a capital city with requisite cultural and civic amenities and architecture"},
{name:"Former Geological Survey of Canada Building ",location:[45.4265139, -75.6939333],Date:"1863 (completion of the oldest part of the building)",Designated:"1955",City:"Ottawa",Type:"PHistoric",Description:"Among the oldest government buildings in the capital, the building was the first Ottawa home of the Geological Survey of Canada; the building was also used to host the inaugural exhibit of the Canadian Academy of Arts in 1880 (the genesis of the collection of the National Gallery of Canada), and to display the Geological Survey's museum collections (which served as the foundation of the Canadian Museum of Nature)"},
{name:"Former Ottawa Teachers' College ",location:[45.4199917, -75.6909306],Date:"1875 (completed)",Designated:"1974",City:"Ottawa",Type:"PHistoric",Description:"A nationally significant example of the Gothic Revival style in an eclectic design; the building served as a normal school until 1974, and now serves as a wing of Ottawa City Hall"},
{name:"Hart Massey House",location:[45.451356, -75.671683],Date:"1959 (completed)",Designated:"2018",City:"Ottawa",Type:"PHistoric",Description:"Built in 1959, Hart Massey House is an iconic example of mid-20th century modernism in residential architecture, and an example of International style because of its sensitivity to its natural surroundings."},
{name:"John R. Booth Residence ",location:[45.416762, -75.692480],Date:"1909 (completed)",Designated:"1990",City:"Ottawa",Type:"PHistoric",Description:"Built for lumber baron John R. Booth, the house is a nationally significant example of the Queen Anne Revival style in domestic architecture"},
{name:"Langevin Block ",location:[45.4236750, -75.6970611],Date:"1889 (completed)",Designated:"1977",City:"Ottawa",Type:"PHistoric",Description:"One of the finest examples of Second Empire style office building architecture in Canada, the Langevin Block was the first purpose-built office building erected by the federal government outside the boundaries of Parliament Hill; currently serves as home to the Prime Minister's Office and the Privy Council Office"},
{name:"Laurier House ",location:[45.4277, -75.6781],Date:"1878 (completed)",Designated:"1956",City:"Ottawa",Type:"PHistoric",Description:"As Canada did not have official residences for elected officials until 1950, this house was the home of Sir Wilfrid Laurier and then William Lyon Mackenzie King during the periods when each was leader of the Liberal Party of Canada; Laurier and King each served both as Prime Ministers and Leaders of the Opposition while living in this home"},
{name:"Maplelawn & Gardens ",location:[45.3882306, -75.7619500],Date:"1834 (completed)",Designated:"1989",City:"Ottawa",Type:"PHistoric",Description:"A rare example of an early 19th-century country estate in Canada and an excellent example of the British classical style; the gardens are the best preserved of the few surviving examples of 19th-century walled gardens in Canada"},
{name:"National Arts Centre ",location:[45.4230, -75.6938],Date:"1969 (completed)",Designated:"2006",City:"Ottawa",Type:"PHistoric",Description:"Built to commemorate the Canadian Centennial and designed in the shape of hexagons, the centre heralded Canada's cultural and architectural achievements in the second half of the 20th century; also a component of the Confederation Square National Historic Site of Canada"},
{name:"Notre-Dame Roman Catholic Basilica ",location:[45.42971, -75.69646],Date:"1842-1897 (construction)",Designated:"1990",City:"Ottawa",Type:"PHistoric",Description:"A Basilica prominently located on Sussex Drive, whose twin towers mark the entrance to Lower Town, one of Ottawa's earliest neighbourhoods; it is considered an exceptional example of Gothic Revival architecture in Canada"},
{name:"Parliament Buildings ",location:[45.4248, -75.6992],Date:"1865 (West Block completed), 1866 (East Block), 1876 (Library of Parliament), 1920 (Centre Block)",Designated:"1976",City:"Ottawa",Type:"PHistoric",Description:"The seat of the Parliament of Canada in a striking location on a hill above the Ottawa River; an important symbol serving as the physical embodiment of the Canadian government and federation"},
{name:"Public Grounds of the Parliament Buildings ",location:[45.424807, -75.699234],Date:"1875 (initial completion)",Designated:"1976",City:"Ottawa",Type:"PHistoric",Description:"The focal point for national celebrations in Ottawa; the grounds were originally designed by Calvert Vaux, and since supplemented by 18 monuments and memorials"},
{name:"Rideau Canal ",location:[45.42583, -75.69722],Date:"1837 (completed)",Designated:"1925",City:"Ottawa to Kingston",Type:"PHistoric",Description:"Built for the British government by Lieutenant-Colonel John By as a defensive work in the event of war with the United States, the still-functioning canal is the best preserved example of a 19th-century slack water canal in North America, with most of its original structures intact; a World Heritage Site and a unique historic landmark in the central core of Ottawa"},
{name:"Rideau Hall and Landscaped Grounds ",location:[45.4443694, -75.685972],Date:"1838 (completion of original villa)",Designated:"1977",City:"Ottawa",Type:"PHistoric",Description:"The official Ottawa residence of the Governor General of Canada and the Canadian monarch; excellent example of the transposition of the natural style of the English country estate to Canada"},
{name:"Royal Canadian Mint ",location:[45.4311500, -75.6992667],Date:"1908 (completion of main building)",Designated:"1979",City:"Ottawa ",Type:"PHistoric",Description:"Representative of the federal government's approach to using the Tudor Gothic architectural style to create a distinctive identity in Canada's capital; the construction of this building, combining a mint and a refinery for gold produced by Canadian mines, symbolized the patriation of control over Canada's currency from Britain"},
{name:"Victoria Memorial Museum ",location:[45.41266, -75.68875],Date:"1911 (completed)",Designated:"1990",City:"Ottawa ",Type:"PHistoric",Description:"Built to house Canada's first national museum, the building originally served as the home of the National Gallery of Canada and of the geological and natural history collections of the Geological Survey of Canada, and then served as the temporary premises of the Parliament of Canada from 1916 to 1922 when the original Centre Block was destroyed by fire; now the home of the Canadian Museum of Nature"},
{name:"Annesley Hall ",location:[43.66778, -79.39306],Date:"1903 (completed)",Designated:"1990",City:"Toronto",Type:"PHistoric",Description:"The first purpose-built women's residence on a Canadian university campus, and a good example of the Queen Anne Revival style in institutional architecture"},
{name:"Balmoral Fire Hall ",location:[43.685833, -79.393870],Date:"1911 (completed)",Designated:"1990",City:"Toronto",Type:"PHistoric",Description:"A rare example of the Queen Anne Revival style used for a fire hall"},
{name:"Bank of Upper Canada Building ",location:[43.6518167, -79.370972],Date:"1825 (completed)",Designated:"1977",City:"Toronto",Type:"PHistoric",Description:"An early 19th-century bank building, representative of the rise of Toronto as a commercial centre and the role played by the Bank of Upper Canada in the development of Upper Canada"},
{name:"Bead Hill",location:[43.8041028, -79.140111],Date:"1600s (village established)",Designated:"1991 (designated);June 15, 2019 (added to national park system)",City:"Toronto",Type:"PHistoric",Description:"An archaeological site in Rouge Park with the only known intact remains of a 17th-century Seneca village in the country"},
{name:"Birkbeck Building ",location:[43.6507056, -79.3779139],Date:"1908 (completed)",Designated:"1986",City:"Toronto",Type:"PHistoric",Description:"A four-storey office building typical of the premises of many financial institutions prevalent in central business districts of Canadian cities before the First World War; representative of a transitional building from that period which combined historical styles with (then) modern design and construction techniques"},
{name:"Eaton's 7th Floor Auditorium and Round Room ",location:[43.660929, -79.383302],Date:"1930 (completed)",Designated:"1983",City:"Toronto",Type:"PHistoric",Description:"A foyer, restaurant and auditorium, designed by French architect Jacques Carlu and muralist Natacha Carlu, located within the former Eaton's College Street department store; remarkable examples of Art Deco and Streamline Moderne interior design"},
{name:"Chapel of St. James-the-Less Anglican Church ",location:[43.669556, -79.3689778],Date:"1861 (completed)",Designated:"1990",City:"Toronto",Type:"PHistoric",Description:"The Chapel is a noted example of High Victorian Gothic Revival architecture and exemplifies the small chapels built in this style in Canada"},
{name:"Eglinton Theatre ",location:[43.7044333, -79.4107389],Date:"1936 (completed)",Designated:"1993",City:"Toronto",Type:"PHistoric",Description:"A cinema representing one of the best examples of the Art Deco-style in Canadian theatre design"},
{name:"Elgin and Winter Garden Theatres ",location:[43.65306, -79.37917],Date:"1914 (completed)",Designated:"1982",City:"Toronto",Type:"PHistoric",Description:"A pair of stacked theatres built by renowned theatre-designer Thomas W. Lamb; originally built for vaudeville, they are the last remaining operational double-decker theatres in the world"},
{name:"Fort York ",location:[43.6390278, -79.40333],Date:"1793 (established), 1815 (current fort completed)",Designated:"1923",City:"Toronto",Type:"PHistoric",Description:"The birthplace of the settlement that would become Toronto and the primary defence for (what was then) York, Upper Canada, the Fort now serves as a museum containing the largest collection of War of 1812 buildings in Canada and many of the oldest buildings in Toronto"},
{name:"Fourth York Post Office ",location:[43.6518472, -79.3706500],Date:"1835 (completed)",Designated:"1980",City:"Toronto",Type:"PHistoric",Description:"Also known as the (First Toronto Post Office) (it was the fourth post office in York, but the first one to serve the settlement when it became Toronto in 1834), it is one of the earliest surviving examples in Canada of a building purpose-built as a post office; typical of small, early 19th-century public buildings, combining public offices and a private residence"},
{name:"George Brown House ",location:[43.655825, -79.39502],Date:"1877 (completed)",Designated:"1976",City:"Toronto",Type:"PHistoric",Description:"The residence of George Brown, founder of (what is now) The Globe and Mail and a Father of Confederation; the site in Toronto most associated with the abolitionist movement and the Underground Railroad"},
{name:"Gooderham and Worts Distillery ",location:[43.65073000, -79.3597694],Date:"1859 to 1927 (construction of extant distillery buildings)",Designated:"1988",City:"Toronto",Type:"PHistoric",Description:"Forty historic distillery buildings on a 13-acre site, representative of the history of the Canadian distilling industry and Toronto's industrial past"},
{name:"Gouinlock Buildings / Early Exhibition Buildings ",location:[43.63278, -79.41611],Date:"1904 to 1912 (completed)",Designated:"1988",City:"Toronto",Type:"PHistoric",Description:"Five buildings (the Fire Hall/Police Station, Government Building, Horticulture Building, Music Building and Press Building) on the grounds of the Canadian National Exhibition; the largest and finest group of early 20th century exhibition buildings in Canada"},
{name:"Heliconian Hall ",location:[43.6719528, -79.39334],Date:"1876 (completed)",Designated:"2008",City:"Toronto",Type:"PHistoric",Description:"Originally constructed as a church in Yorkville in the Carpenter Gothic style, the building has since 1923 served as a unique multidisciplinary arts club specifically for women"},
{name:"John Street Roundhouse (Canadian Pacific) ",location:[43.640862, -79.385925],Date:"1931 (completed)",Designated:"1990",City:"Toronto",Type:"PHistoric",Description:"Located in Torontos formerly vast railway lands near Union Station, it is Canada's best surviving example of a roundhouse; now occupied by the Toronto Railway Heritage Centre, the Steam Whistle brewery and a furniture store"},
{name:"Kensington Market ",location:[43.6547722, -79.4006778],Date:"1815 (first development (Bellevue Estate))",Designated:"2006",City:"Toronto",Type:"PHistoric",Description:"A neighbourhood noted for its network of narrow streets and lanes fronted by rows of small houses and shops; since the early 20th century, it has been home to numerous successive waves of immigrant communities, making it a microcosm of Canada's multiculturalism"},
{name:"Maple Leaf Gardens ",location:[43.66222, -79.38028],Date:"1931 (completed)",Designated:"2007",City:"Toronto",Type:"PHistoric",Description:"Built for the Toronto Maple Leafs, the arena is regarded as of the most renowned (shrines) in the history of ice hockey; for 70 years, it was one of Canada's foremost venues for large-scale sporting events, concerts and political events"},
{name:"Massey Hall ",location:[43.65417, -79.3790278],Date:"1894 (completed)",Designated:"1981",City:"Toronto",Type:"PHistoric",Description:"A gift to the City of Toronto from wealthy industrialist Hart Massey, the concert hall has been one of the country's most important cultural institutions and is renowned for its outstanding acoustics"},
{name:"Metallic Roofing Company Offices ",location:[43.6391694, -79.4271556],Date:"1897 (completed)",Designated:"1984",City:"Toronto",Type:"PHistoric",Description:"A unique Beaux-Arts style building decorated entirely in pressed metal; the building was dismantled in 1982 when the site was redeveloped, and it is held by the Ontario Heritage Trust for eventual reconstruction"},
{name:"Montgomery's Tavern ",location:[43.7095, -79.3990],Date:"1837 (battle)",Designated:"1925",City:"Toronto",Type:"PHistoric",Description:"The site of an abortive insurrection by William Lyon MacKenzie during the Upper Canada Rebellion; the rebellion ultimately contributed to the establishment of responsible government in the colony"},
{name:"Mount Pleasant Cemetery ",location:[43.696351, -79.384882],Date:"1876 (opened)",Designated:"2000",City:"Toronto",Type:"PHistoric",Description:"An outstanding example of picturesque design inspired by the 19th-century tradition of rural cemeteries in a naturalistic setting; many of the grave markers are representative of significant epochs in the history of Toronto and the rest of the country"},
{name:"Old Toronto City Hall and York County Court House ",location:[43.65250, -79.38167],Date:"1899 (completed)",Designated:"1984",City:"Toronto",Type:"PHistoric",Description:"One of Canadas finest examples of Richardsonian Romanesque architecture and a symbol of Toronto's prosperity and rapid urbanization in the late 19th century"},
{name:"Old Toronto Post Office / Old Bank of Canada ",location:[43.64999, -79.376355],Date:"1853 (completed)",Designated:"1958",City:"Toronto",Type:"PHistoric",Description:"A noted example of Greek Revival architecture in Canada"},
{name:"Osgoode Hall ",location:[43.65222, -79.38556],Date:"1832 (original wing completed)",Designated:"1979",City:"Toronto",Type:"PHistoric",Description:"Housing the Law Society of Upper Canada, courts of law and, until 1959, the only law school in the province, Osgoode Hall symbolizes the legal profession and court system in Ontario; a landmark on Queen Street West, it is also known for its ornate interiors"},
{name:"Royal Alexandra Theatre ",location:[43.64741, -79.38750],Date:"1907 (completed)",Designated:"1985",City:"Toronto",Type:"PHistoric",Description:"One of the last theatres of its type built in Canada and arguably the best surviving example, it is a nationally significant theatre that has played a central role in the social and cultural life of Toronto"},
{name:"Royal Conservatory of Music ",location:[43.667972, -79.3962500],Date:"1881 (Ihnatowycz Hall completed)",Designated:"1995",City:"Toronto",Type:"PHistoric",Description:"Originally constructed as the first home of McMaster University, Ihnatowycz Hall has housed the Royal Conservatory of Music since 1962; some of Canada's some of the most prominent musicians and music teachers have studied at the Conservatory, and it has played a significant role in music education across the country"},
{name:"St. Anne's Anglican Church ",location:[43.6506222, -79.4306528],Date:"1908 (completed)",Designated:"1996",City:"Toronto",Type:"PHistoric",Description:"The church contains a unique cycle of paintings, executed in 1923, by ten noted artists, including three members of the Group of Seven, under the supervision of J. E. H. MacDonald"},
{name:"St. George's Hall (Arts and Letters Club) ",location:[43.6578139, -79.3826417],Date:"1891 (completed)",Designated:"2007",City:"Toronto",Type:"PHistoric",Description:"Since 1920, St. George's Hall has been a gathering place for painters, writers, musicians, architects, actors and patrons of the arts; an important venue and catalyst for artistic activity in Canada"},
{name:"St. Lawrence Hall ",location:[43.65028, -79.37222],Date:"1850 (completed)",Designated:"1967",City:"Toronto",Type:"PHistoric",Description:"St. Lawrence Hall was for many years Toronto's chief social and cultural centre, and is among the finest 19th century public buildings in Canada"},
{name:"The Grange ",location:[43.6531250, -79.392417],Date:"1817 (completed)",Designated:"1970",City:"Toronto",Type:"PHistoric",Description:"An historic Georgian manor in downtown Toronto, it is one of the few surviving residential estates belonging to prominent citizens from the settlement of York; the oldest remaining brick house in Toronto"},
{name:"The Studio Building ",location:[43.67325, -79.386083],Date:"1914 (completed)",Designated:"2005",City:"Toronto",Type:"PHistoric",Description:"An early Canadian artists studio in the modernist style, with associations with important Canadian artists including the Group of Seven; designed by architect Eden Smith for painter Lawren Harris, it contains six purpose-built studio spaces and, at one time, artists such as Tom Thomson, Arthur Lismer and Thoreau MacDonald lived and worked on site"},
{name:"Toronto Island Airport Terminal Building ",location:[43.6319472, -79.3957361],Date:"1939 (completed)",Designated:"1989",City:"Toronto",Type:"PHistoric",Description:"Among the first group of airport terminals to be funded and approved by the then new Department of Transport as part of the development of Trans-Canada Air Lines; one of very few early terminal buildings to have survived and likely the oldest operating terminal of its kind in the country"},
{name:"Union Station (Canadian Pacific and Grand Trunk) ",location:[43.64528, -79.38056],Date:"1927 (completed)",Designated:"1975",City:"Toronto",Type:"PHistoric",Description:"The finest example in Canada of classical Beaux-Arts railway stations, and the largest of the great urban stations built in the country during the early 20th century; illustrative of an era when railways were expanding and Toronto was becoming a modern metropolis"},
{name:"University College ",location:[43.66222, -79.39583],Date:"1859 (completed)",Designated:"1968",City:"Toronto",Type:"PHistoric",Description:"One of the oldest collegiate buildings in the country, associated both with the development of non-denominational, publicly supported institutions of higher education in Canada, and with the development of the University of Toronto"},
{name:"Women's College Hospital ",location:[43.661686, -79.387497],Date:"1883 (founded)",Designated:"1995",City:"Toronto",Type:"PHistoric",Description:"Founded at a time when women's access to medical education and hospital practice was extremely restricted, the hospital uniquely emphasizes women's health issues and women as health care providers; symbolic of the struggle and contribution of Canadian women to the medical profession"}
];

const parks = [{name:"Aaron Provincial Park",location:[49.760403100000005,-92.65614559999999],Type:"Provintial _Park"},
{name:"Algonquin Provincial Park",location:[45.8371591,-78.37912390000001],Type:"Provintial _Park"},
{name:"Arrowhead Provincial Park",location:[45.3915527,-79.21467829999999],Type:"Provintial _Park"},
{name:"Arrow Lake Provincial Park",location:[48.176964,-90.2272712],Type:"Provintial _Park"},
{name:"Awenda Provincial Park",location:[44.8358315,-79.9966048],Type:"Provintial _Park"},
{name:"Balsam Lake Provincial Park",location:[44.63593529999999,-78.8752853],Type:"Provintial _Park"},
{name:"Bass Lake Provincial Park",location:[44.602397700000004,-79.4854253],Type:"Provintial _Park"},
{name:"Batchawana Bay Provincial Park",location:[46.9409303,-84.54878120000001],Type:"Provintial _Park"},
{name:"Biscotasi Lake Provintial Park",location:[47.3075278,-82.0086661],Type:"Provintial _Park"},
{name:"Blue Lake Provincial Park",location:[49.9034135,-93.4726929],Type:"Provintial _Park"},
{name:"Bon Echo Provincial Park",location:[44.8971568,-77.2091615],Type:"Provintial _Park"},
{name:"Bonnechere Provincial Park",location:[45.662824799999996,-77.5708995],Type:"Provintial _Park"},
{name:"Bronte Creek Provincial Park (Day use Entrance)",location:[43.4063552,-79.771315],Type:"Provintial _Park"},
{name:"Caliper Lake Provincial Park",location:[49.063361900000004,-93.91379350000001],Type:"Provintial _Park"},
{name:"Charleston Lake Provincial Park",location:[44.5027778,-76.0405556],Type:"Provintial _Park"},
{name:"Chutes Provincial Park",location:[46.224665,-82.0729189],Type:"Provintial _Park"},
{name:"Craigleith Provincial Park",location:[44.53667120000001,-80.3484007],Type:"Provintial _Park"},
{name:"Darlington Provincial Park",location:[43.8725539,-78.77878890000001],Type:"Provintial _Park"},
{name:"Driftwood Provincial Park",location:[46.1859099,-77.84678829999999],Type:"Provintial _Park"},
{name:"Earl Rowe Provincial Park",location:[44.149817600000006,-79.8970235],Type:"Provintial _Park"},
{name:"Emily Provincial Park",location:[44.3367813,-78.5352625],Type:"Provintial _Park"},
{name:"Esker Lakes Provincial Park",location:[48.323479799999994,-79.8787322],Type:"Provintial _Park"},
{name:"Fairbank Provincial Park",location:[46.472178799999995,-81.43956899999999],Type:"Provintial _Park"},
{name:"Ferris Provincial Park",location:[44.2913461,-77.7938522],Type:"Provintial _Park"},
{name:"Finlayson Point Provincial Park",location:[47.054653200000004,-79.80498809999999],Type:"Provintial _Park"},
{name:"Fitzroy Provincial Park",location:[45.483053999999996,-76.2184575],Type:"Provintial _Park"},
{name:"Forks of the Credit Provincial Park",location:[43.825070700000005,-80.00415770000001],Type:"Provintial _Park"},
{name:"French River Provincial Park",location:[45.899167,-80.8398541],Type:"Provintial _Park"},
{name:"Frontenac Provincial Park",location:[44.5076716,-76.5531634],Type:"Provintial _Park"},
{name:"Fushimi Lake Provincial Park",location:[49.828591700000004,-83.9171696],Type:"Provintial _Park"},
{name:"Grundy Lake Provincial Park",location:[45.916712200000006,-80.5595235],Type:"Provintial _Park"},
{name:"Halfway Lake Provincial Park",location:[46.9005885,-81.6585715],Type:"Provintial _Park"},
{name:"Inverhuron Provincial Park",location:[44.2997333,-81.5880866],Type:"Provintial _Park"},
{name:"Ivanhoe Lake Provincial Park",location:[48.174030200000004,-82.50186059999999],Type:"Provintial _Park"},
{name:"John E. Pearce Provincial Park",location:[42.6088089,-81.4457522],Type:"Provintial _Park"},
{name:"Ontario Kakabeka Falls Provintial Park",location:[48.3971359,-89.63243],Type:"Provintial _Park"},
{name:"Kap-Kig-Iwan Provincial Park",location:[47.8006711,-79.8803462],Type:"Provintial _Park"},
{name:"Kawartha Highlands Provincial Park",location:[44.745952,-78.2200798],Type:"Provintial _Park"},
{name:"Kettle Lakes Provincial Park",location:[48.56928920000001,-80.8665532],Type:"Provintial _Park"},
{name:"Killarney Provincial Park",location:[46.0130472,-81.4017498],Type:"Provintial _Park"},
{name:"Killbear Provincial Park",location:[45.3586283,-80.21332840000001],Type:"Provintial _Park"},
{name:"Komoka Provincial Park",location:[42.9530318,-81.3840124],Type:"Provintial _Park"},
{name:"Lady Evelyn-Smoothwater Provincial Park",location:[47.3741999,-80.5191204],Type:"Provintial _Park"},
{name:"Lake on the Mountain Provincial Park",location:[44.0398731,-77.05604050000001],Type:"Provintial _Park"},
{name:"Lake Saint Peter Provincial Park",location:[45.3378006,-78.02266850000001],Type:"Provintial _Park"},
{name:"Lake Superior Provincial Park",location:[47.7510684,-84.83591080000001],Type:"Provintial _Park"},
{name:"Long Point Provincial Park",location:[42.581664399999994,-80.3951704],Type:"Provintial _Park"},
{name:"MacGregor Point Provincial Park",location:[44.4139114,-81.45526559999999],Type:"Provintial _Park"},
{name:"MacLeod Provincial Park",location:[49.689204,-86.8996405],Type:"Provintial _Park"},
{name:"Lady Evelyn-Smoothwater Provincial Park",location:[47.3741999,-80.5191204],Type:"Provintial _Park"},
{name:"Mara Provincial Park",location:[44.587560100000005,-79.35905100000001],Type:"Provintial _Park"},
{name:"Mark S. Burnham Provincial Park",location:[44.3008411,-78.2689714],Type:"Provintial _Park"},
{name:"Marten River Provincial Park",location:[46.7331131,-79.8027455],Type:"Provintial _Park"},
{name:"McRae Point Provincial Park",location:[44.5680571,-79.3304106],Type:"Provintial _Park"},
{name:"Mikisew Provincial Park",location:[45.8214037,-79.51312469999999],Type:"Provintial _Park"},
{name:"Misery Bay Provincial Nature Reserve",location:[45.7921839,-82.7354934],Type:"National_Park"},
{name:"Missinaibi Provincial Park",location:[48.404508899999996,-83.522217],Type:"Provintial _Park"},
{name:"Mississagi Provincial Park",location:[46.6006735,-82.6828646],Type:"Provintial _Park"},
{name:"Mono Cliffs Provincial Park",location:[44.0477948,-80.0696976],Type:"Provintial _Park"},
{name:"Murphys Point Provincial Park",location:[44.7818636,-76.23747159999999],Type:"Provintial _Park"},
{name:"Nagagamisis Provincial Park",location:[49.513579799999995,-84.80706909999999],Type:"Provintial _Park"},
{name:"Neys Provincial Park",location:[48.770205700000005,-86.5815353],Type:"Provintial _Park"},
{name:"North Beach Provincial Park",location:[43.9577511,-77.5257968],Type:"Provintial _Park"},
{name:"Oastler Lake Provincial Park",location:[45.3113527,-79.96430129999999],Type:"Provintial _Park"},
{name:"Obabika River Provincial Park",location:[47.140617299999995,-80.3085149],Type:"Provintial _Park"},
{name:"Ojibway Provincial Park",location:[49.983639600000004,-92.14239040000001],Type:"Provintial _Park"},
{name:"Ouimet Canyon Provincial Park",location:[48.789513899999996,-88.6712946],Type:"Provintial _Park"},
{name:"Oxtongue River - Ragged Falls Provincial Park",location:[45.39067070000001,-78.90798290000001],Type:"Provintial _Park"},
{name:"Pakwash Provincial Park",location:[50.7844542,-93.44570870000001],Type:"Provintial _Park"},
{name:"Pancake Bay Provincial Park",location:[46.9693434,-84.6871068],Type:"Provintial _Park"},
{name:"Petroglyphs Provincial Park",location:[44.621559000000005,-78.044014],Type:"Provintial _Park"},
{name:"Pigeon River Provincial Park",location:[48.00512,-89.59529870000001],Type:"Provintial _Park"},
{name:"Pinery Provincial Park",location:[43.2482354,-81.8224354],Type:"Provintial _Park"},
{name:"Point Farms Provincial Park",location:[43.8013554,-81.700959],Type:"Provintial _Park"},
{name:"Port Bruce Provincial Park",location:[42.654795899999996,-81.0130515],Type:"Provintial _Park"},
{name:"Port Burwell Provincial Park",location:[42.641000299999995,-80.8106862],Type:"Provintial _Park"},
{name:"Presqu'ile Provincial Park",location:[44.00980070000001,-77.7420364],Type:"Provintial _Park"},
{name:"Quetico Provincial Park",location:[48.6749651,-91.1259929],Type:"Provintial _Park"},
{name:"Rainbow Falls Provincial Park, Rossport Campground",location:[48.843837,-87.3959376],Type:"Provintial _Park"},
{name:"RenÃ© Brunelle Provincial Park",location:[49.4546556,-82.1375324],Type:"Provintial _Park"},
{name:"Restoule Provincial Park",location:[46.0732855,-79.78313670000001],Type:"Provintial _Park"},
{name:"Rideau River Provincial Park",location:[45.0599809,-75.67072859999999],Type:"Provintial _Park"},
{name:"Rock Point Provincial Park",location:[42.849149,-79.55191429999999],Type:"Provintial _Park"},
{name:"Rondeau Provincial Park",location:[42.3173601,-81.8470973],Type:"Provintial _Park"},
{name:"Rushing River Provincial Park - Northwest Campground",location:[49.686908200000005,-94.2239327],Type:"Provintial _Park"},
{name:"Samuel de Champlain Provincial Park",location:[46.2839009,-78.8544047],Type:"Provintial _Park"},
{name:"Sandbanks Provincial Park",location:[43.9070266,-77.2392234],Type:"Provintial _Park"},
{name:"Sandbar Lake Provincial Park",location:[49.4675848,-91.54543199999999],Type:"Provintial _Park"},
{name:"Sauble Falls Provincial Park",location:[44.6738004,-81.25864849999999],Type:"Provintial _Park"},
{name:"Selkirk Provincial Park",location:[42.8158039,-79.9578763],Type:"Provintial _Park"},
{name:"Sharbot Lake Provincial Park",location:[44.773914899999994,-76.72472940000002],Type:"Provintial _Park"},
{name:"Sibbald Point Provincial Park",location:[44.321956400000005,-79.3254315],Type:"Provintial _Park"},
{name:"Silent Lake Provincial Park",location:[44.92319570000001,-78.0712131],Type:"Provintial _Park"},
{name:"Silver Falls Provincial Park",location:[48.6884314,-89.6270378],Type:"Provintial _Park"},
{name:"Silver Lake Provincial Park",location:[44.832033200000005,-76.57646770000001],Type:"Provintial _Park"},
{name:"Sioux Narrows Provincial Park",location:[49.42641629999999,-94.0477367],Type:"Provintial _Park"},
{name:"Six Mile Lake Provintial Park",location:[44.8888148,-79.74982840000001],Type:"Provintial _Park"},
{name:"Sleeping Giant Provincial Park",location:[48.36930460000001,-88.804575],Type:"Provintial _Park"},
{name:"Solace Provincial Park",location:[47.1984076,-80.69357740000001],Type:"Provintial _Park"},
{name:"Spanish River Provincial Park",location:[46.7010281,-81.7288613],Type:"Provintial _Park"},
{name:"Springwater Provincial Park",location:[44.4386085,-79.7622105],Type:"Provintial _Park"},
{name:"Sturgeon Bay Provincial Park",location:[45.624957200000004,-80.41426240000001],Type:"Provintial _Park"},
{name:"Sturgeon River Provincial Park",location:[46.72031629999999,-80.3639815],Type:"Provintial _Park"},
{name:"The Massasauga Provincial Park",location:[45.2641219,-80.00826479999999],Type:"Provintial _Park"},
{name:"Tidewater Provincial Park",location:[51.2627966,-80.6241543],Type:"Provintial _Park"},
{name:"Turkey Point Provincial Park",location:[42.704550700000006,-80.334486],Type:"Provintial _Park"},
{name:"Voyageur Provincial Park",location:[45.55831,-74.4518921],Type:"Provintial _Park"},
{name:"Wabakimi Provincial Park",location:[50.7578861,-89.5369022],Type:"Provintial _Park"},
{name:"Wakami Lake Provincial Park",location:[47.4816312,-82.846108],Type:"Provintial _Park"},
{name:"Wasaga Beach Provincial Park",location:[44.5254227,-80.0166578],Type:"Provintial _Park"},
{name:"Wheatley Provincial Park",location:[42.0886953,-82.4454857],Type:"Provintial _Park"},
{name:"White Lake Provincial Park Campground",location:[48.6951118,-85.6460947],Type:"Provintial _Park"},
{name:"Windy Lake Provincial Park",location:[46.6191484,-81.4461277],Type:"Provintial _Park"},
{name:"Woodland Caribou Provincial Park",location:[51.0790067,-94.85076579999999],Type:"Provintial _Park"},
{name:"Thousand Islands National Park",location:[44.452962400000004,-75.86088459999999],Type:"National_Park"},
{name:"Point Pelee National Park Of Canada",location:[41.962771999999994,-82.51844],Type:"National_Park"},
{name:"Georgian Bay Islands National Park",location:[44.8743165,-79.8699554],Type:"National_Park"},
{name:"Pukaskwa National Park",location:[48.245678399999996,-85.8877814],Type:"National_Park"},
{name:"Bruce Peninsula National Park",location:[45.2288114,-81.5232556],Type:"National_Park"}
];

const cultedu = [{name:"African Lion Safari",location:[43.34249115,-80.27240753],City:"Cambridge",Type:"Zoos"},
{name:"Springridge Farm",location:[43.4843048,-79.9059813],City:"Milton",Type:"Zoos"},
{name:"Twin Valley Animal Park",location:[43.15668,-80.12974],City:"Brantford",Type:"Zoos"},
{name:"High Park",location:[43.64762816,-79.46393967],City:"Toronto",Type:"Zoos"},
{name:"Killman Zoo",location:[43.10385,-79.91346],City:"Caledonia",Type:"Zoos"},
{name:"Reptilia",location:[43.834518,-79.516441],City:"Concord",Type:"Zoos"},
{name:"High Park Zoo",location:[43.64272329999999,-79.4612017],City:"Toronto",Type:"Zoos"},
{name:"Ostrich Land",location:[43.03848921,-79.64062082],City:"Caistor Centre",Type:"Zoos"},
{name:"Greenview Aviaries",location:[42.42035,-81.8428],City:"Ridgetown",Type:"Zoos"},
{name:"Indian River Reptile Zoo",location:[44.344190399999995,-78.0867798],City:"Norwood",Type:"Zoos"},
{name:"Pine Ridge Ranch",location:[46.664780799999996,-80.88175290000001],City:"Hanmer",Type:"Zoos"},
{name:"Parc Omega",location:[45.682601,-74.943811],City:"Montebello",Type:"Zoos"},
{name:"Zoo Ecomuseum",location:[45.425548,-73.93478499999999],City:"Sainte-Anne-de-Bellevue",Type:"Zoos"},
{name:"Papanack Zoo",location:[45.557323499999995,-75.11891170000001],City:"Wendover",Type:"Zoos"},
{name:"Exotarium",location:[45.56563,-73.98868459999998],City:"Saint-Eustache",Type:"Zoos"},
{name:"Parc-Nature Cap-Saint-Jacques",location:[45.471804,-73.93589],City:"Pierrefonds",Type:"Zoos"},
{name:"Vanderlaand Barnyard Zoo",location:[45.01433568,-75.33679314],City:"Inkerman",Type:"Zoos"},
{name:"Little Ray's Reptile Zoo",location:[45.2884029,-75.5744464],City:"Ottawa",Type:"Zoos"},
{name:"Valleyview Little Animal Farm",location:[45.264829999999996,-75.81341],City:"Nepean",Type:"Zoos"},
{name:"Niagara Parks Butterfly Conservatory",location:[43.136989,-79.0556101],City:"Niagara Falls",Type:"Zoos"},
{name:"Bird Kingdom",location:[43.092690000000005,-79.06855999999999],City:"Niagara Falls",Type:"Zoos"},
{name:"Toronto Zoo",location:[43.817703,-79.18589200000001],City:"Toronto",Type:"Zoos"},
{name:"Riverdale Farm",location:[43.667319899999995,-79.3610099],City:"Toronto",Type:"Zoos"},
{name:"Safari Niagara",location:[42.95147,-79.05544],City:"Fort Erie",Type:"Zoos"},
{name:"Centreville Theme Park",location:[43.62043853,-79.37470963],City:"Toronto",Type:"Zoos"},
{name:"Jungle Cat World Wildlife Park",location:[43.99315412,-78.60722065],City:"Orono",Type:"Zoos"},
{name:"Reptilia",location:[43.8652445,-78.90692320000001],City:"Whitby",Type:"Zoos"},
{name:"Havelberg Dog Academy",location:[44.03647,-78.63028],City:"Orono",Type:"Zoos"},
{name:"Colasanti Tropical Gardens",location:[42.0625339,-82.6783264],City:"Kingsville",Type:"Zoos"},
{name:"Elmvale Jungle Zoo",location:[44.549456,-79.8363207],City:"Elmvale",Type:"Zoos"},
{name:"Cochrane Polar Bear Habitat",location:[49.05291285,-81.01724163],City:"Cochrane",Type:"Zoos"},
{name:"Agnes Etherington Art Centre",location:[44.225359999999995,-76.49625999999999],City:"Kingston",Type:"Art Museums"},
{name:"Amazing Science Centre",location:[45.0274051,-74.73219240000002],City:"Cornwall",Type:"Science Centre"},
{name:"Amherstburg Freedom Museum",location:[42.10132,-83.10569],City:"Amherstburg",Type:"Museums"},
{name:"Aquatarium",location:[44.587934000000004,-75.6826267],City:"Brockville",Type:"Aquariums"},
{name:"Art Gallery of Guelph",location:[43.53295538,-80.23293972],City:"Guelph",Type:"Art Museums"},
{name:"Art Gallery of Ontario AGO",location:[43.6536766,-79.39233940000001],City:"Toronto",Type:"Art Museums"},
{name:"Backus Heritage Conservation Area",location:[42.63855743,-80.48214722],City:"Port Rowan",Type:"Museums"},
{name:"Bank of Canada Museum",location:[45.4212817,-75.7016505],City:"Ottawa",Type:"Museums"},
{name:"Banting House National Historic Site",location:[42.99,-81.2318],City:"London",Type:"Museums"},
{name:"Battlefield House",location:[43.21845,-79.76613],City:"Stoney Creek",Type:"Museums"},
{name:"Bell Homestead National Historic Site",location:[43.1076852,-80.27068680000001],City:"Brantford",Type:"Museums"},
{name:"Bellevue House National Historic Site",location:[44.223011,-76.5040817],City:"Kingston",Type:"Museums"},
{name:"Benares Historic House",location:[43.52753,-79.63718],City:"Mississauga",Type:"Museums"},
{name:"Billings Estate Museum",location:[45.3894368,-75.67248509999999],City:"Ottawa",Type:"Museums"},
{name:"Black Creek Pioneer Village",location:[43.77478,-79.51804],City:"Downsview",Type:"Museums"},
{name:"Bradley Museum",location:[43.51208,-79.6114599],City:"Mississauga",Type:"Museums"},
{name:"Brockville Museum",location:[44.5863346,-75.68666809999999],City:"Brockville",Type:"Museums"},
{name:"Campbell House Museum",location:[43.6509,-79.38710999999999],City:"Toronto",Type:"Museums"},
{name:"Canada Agriculture and Food Museum",location:[45.387895,-75.707585],City:"Ottawa",Type:"Museums"},
{name:"Canada Aviation and Space Museum",location:[45.458164200000006,-75.644246],City:"Ottawa",Type:"Museums"},
{name:"Canada Science and Technology Museum",location:[45.40374642,-75.61990349],City:"Ottawa",Type:"Museums"},
{name:"Canadian Automotive Museum",location:[43.8948331,-78.8623571],City:"Oshawa",Type:"Museums"},
{name:"Canadian Baseball Hall of Fame and Museum",location:[43.2499732,-81.14523299999999],City:"St. Marys",Type:"Museums"},
{name:"Canadian Bushpilot Heritage Centre",location:[46.5067276,-84.32297690000001],City:"Sault Ste. Marie",Type:"Museums"},
{name:"Canadian Canoe Museum",location:[44.28845,-78.33025],City:"Peterborough",Type:"Museums"},
{name:"Canadian Football Hall of Fame",location:[43.25225528,-79.83126383],City:"Hamilton",Type:"Museums"},
{name:"Canadian Historical Aircraft Assoc",location:[42.2683611,-82.9683634],City:"Windsor",Type:"Museums"},
{name:"Canadian Museum of Hindu Civilization",location:[43.8382919,-79.42972929999999],City:"Richmond Hill",Type:"Museums"},
{name:"Canadian Museum of Nature",location:[45.41271425,-75.68851456],City:"Ottawa",Type:"Museums"},
{name:"Canadian War Museum",location:[45.417273,-75.716904],City:"Ottawa",Type:"Museums"},
{name:"Canadian Warplane Heritage Museum",location:[43.15996,-79.92489990000001],City:"Mount Hope",Type:"Museums"},
{name:"Casa Loma",location:[43.678142,-79.40943299999999],City:"Toronto",Type:"Museums"},
{name:"Castle Dracula Wax Museum",location:[43.09145,-79.07425],City:"Niagara Falls",Type:"Museums"},
{name:"City of Waterloo Museum",location:[43.498718700000005,-80.5298647],City:"Waterloo",Type:"Museums"},
{name:"Colborne Lodge",location:[43.64060704,-79.46069379],City:"Toronto",Type:"Museums"},
{name:"Collingwood Museum",location:[44.5023848,-80.2146287],City:"Collingwood",Type:"Museums"},
{name:"Cumberland Heritage Village Museum",location:[45.517835,-75.391255],City:"Cumberland",Type:"Museums"},
{name:"Dance Collection Danse",location:[43.653295799999995,-79.3717524],City:"Toronto",Type:"Museums"},
{name:"Daredevil Gallery & Museum",location:[43.08507,-79.08207],City:"Niagara Falls",Type:"Museums"},
{name:"Diefenbunker",location:[45.3550351,-76.0487411],City:"Ottawa",Type:"Museums"},
{name:"Discovery Harbour",location:[44.799397,-79.932139],City:"Penetanguishene",Type:"Museums"},
{name:"Doon Heritage Village",location:[43.4010807,-80.43556579999999],City:"Kitchener",Type:"Museums"},
{name:"Downtown Rideau",location:[45.4259787,-75.6925947],City:"Ottawa",Type:"Museums"},
{name:"Dundurn National Historic Site",location:[43.26853,-79.88501],City:"Hamilton",Type:"Landmarks & Historical Buildings"},
{name:"Dynamic Earth",location:[46.4738324,-81.0340385],City:"Greater Sudbury",Type:"Museums"},
{name:"Entomica",location:[46.5165062,-84.3480225],City:"Sault Ste. Marie",Type:"Museums"},
{name:"Erchless Estate",location:[43.44198,-79.66669],City:"Oakville",Type:"Landmarks & Historical Buildings"},
{name:"Ermatinger Clergue National Historic Site",location:[46.50622,-84.32457],City:"Sault Ste. Marie",Type:"Museums"},
{name:"Fanshawe Pioneer Village",location:[43.054212299999996,-81.1848016],City:"London",Type:"Museums"},
{name:"Fort Malden National Historic Park",location:[42.108129999999996,-83.11255],City:"Amherstburg",Type:"Museums"},
{name:"Fort William Historical Park",location:[48.35302,-89.354],City:"Thunder Bay",Type:"Museums"},
{name:"Fort York: Garrison Common",location:[43.6376309,-79.4048273],City:"Toronto",Type:"Museums"},
{name:"Fulford Place",location:[44.598949299999994,-75.66955440000001],City:"Brockville",Type:"Museums"},
{name:"Gananoque Museum",location:[44.326631899999995,-76.1647145],City:"Gananoque",Type:"Museums"},
{name:"Gibson House Museum",location:[43.7697,-79.41467],City:"North York",Type:"Museums"},
{name:"Glanmore National Historic Site",location:[44.16725,-77.36779],City:"Belleville",Type:"Museums"},
{name:"Goulbourn Museum",location:[45.23404,-75.90554],City:"Stittsville",Type:"Museums"},
{name:"Great War Flying Museum",location:[43.75852311,-79.87887955],City:"Cheltenham",Type:"Museums"},
{name:"Grey Roots Museum & Archive",location:[44.55673,-80.92546999999999],City:"Owen Sound",Type:"Museums"},
{name:"Guinness Book of World Records Museum",location:[43.09153,-79.07344],City:"Niagara Falls",Type:"Museums"},
{name:"Halton County Radial Railway",location:[43.579513,-80.1130239],City:"Milton",Type:"Museums"},
{name:"Hamilton Children's Museum",location:[43.2438511,-79.8256512],City:"Hamilton",Type:"Children's Museums"},
{name:"HideSeek",location:[43.64927,-79.42581],City:"Toronto",Type:"Art Museums"},
{name:"Historic Zion Schoolhouse",location:[43.790167600000004,-79.36601309999999],City:"Toronto",Type:"Museums"},
{name:"Hockey Hall of Fame",location:[43.64695062,-79.37723721],City:"Toronto",Type:"Museums"},
{name:"House Of Frankenstein",location:[43.09219,-79.07514],City:"Niagara Falls",Type:"Museums"},
{name:"Hutchison House Museum",location:[44.307179999999995,-78.32521],City:"Peterborough",Type:"Museums"},
{name:"Ingersoll Cheese Factory and Museum",location:[43.03718,-80.86679000000001],City:"Ingersoll",Type:"Museums"},
{name:"John McKenzie House",location:[43.7714923,-79.4112287],City:"Willowdale",Type:"Museums"},
{name:"Kitchener-Waterloo Art Gallery",location:[43.454274,-80.48465999999999],City:"Kitchener",Type:"Museums"},
{name:"Lang Pioneer Village",location:[44.274390999999994,-78.169708],City:"Keene",Type:"Museums"},
{name:"London Regional Children's Museum",location:[42.979359,-81.26257609999999],City:"London",Type:"Children's Museums"},
{name:"Louis Tussaud's Waxworks",location:[43.09256,-79.07601],City:"Niagara Falls",Type:"Museums"},
{name:"Mackenzie House",location:[43.6556799,-79.37841],City:"Toronto",Type:"Museums"},
{name:"MacPherson House",location:[44.2528,-76.94566],City:"Napanee",Type:"Museums"},
{name:"Mariners Park Museum",location:[43.92224425,-77.0526683],City:"Milford",Type:"Museums"},
{name:"Markham Museum",location:[43.8937944,-79.2646439],City:"Markham",Type:"Museums"},
{name:"Matawa Museum",location:[46.31990682,-78.70938163],City:"Mattawa",Type:"Museums"},
{name:"Military Communications And Electronics Museum",location:[44.2422295,-76.4397964],City:"Kingston",Type:"Museums"},
{name:"Montgomery's Inn Museum",location:[43.65168,-79.52561999999999],City:"Etobicoke",Type:"Museums"},
{name:"Movieland Wax Museum Of The Stars",location:[43.09125926,-79.07468126],City:"Niagara Falls",Type:"Museums"},
{name:"MusÃ©e Canadien de l'Histoire",location:[45.430356599999996,-75.7098106],City:"Gatineau",Type:"Museums"},
{name:"Museum London",location:[42.9826,-81.2548],City:"London",Type:"Art Galleries"},
{name:"Museum Of Contemporary Art",location:[43.65455719,-79.44513615],City:"Toronto",Type:"Museums"},
{name:"Museum of Health Care",location:[44.223800700000005,-76.49215699999999],City:"Kingston",Type:"Museums"},
{name:"Museum of Illusions",location:[43.650114,-79.369661],City:"Toronto",Type:"Museums"},
{name:"Museum of Ontario Archaeology",location:[43.01355,-81.3055],City:"London",Type:"Museums"},
{name:"MZTV Museum of Television",location:[43.637640000000005,-79.42165],City:"Toronto",Type:"Museums"},
{name:"National Air Force Museum of Canada",location:[44.1158476,-77.5507476],City:"Trenton",Type:"Museums"},
{name:"National Gallery of Canada",location:[45.429584999999996,-75.69895799999999],City:"Ottawa",Type:"Art Galleries"},
{name:"Nepean Museum",location:[45.34896,-75.74181],City:"Nepean",Type:"Museums"},
{name:"Niagara Falls History Museum",location:[43.10114172,-79.07346094],City:"Niagara Falls",Type:"Museums"},
{name:"Niagara Military Museum",location:[43.10113,-79.07378],City:"Niagara Falls",Type:"Museums"},
{name:"Oakville Galleries",location:[43.46117413,-79.64761734],City:"Oakville",Type:"Museums"},
{name:"Oakville Galleries at Centennial Square",location:[43.4443661,-79.67060359999999],City:"Oakville",Type:"Art Galleries"},
{name:"Oil Museum Of Canada",location:[42.773309999999995,-82.12089],City:"Oil Springs",Type:"Museums"},
{name:"Ojibwa Submarine",location:[42.646280700000005,-80.80776226],City:"Port Burwell",Type:"Museums"},
{name:"Ontario Regiment Museum",location:[43.91855411,-78.89180638],City:"Oshawa",Type:"Museums"},
{name:"Ontario Science Centre",location:[43.7167223,-79.33887711],City:"Toronto",Type:"Science Centre"},
{name:"Ottawa Insectarium",location:[45.423429999999996,-75.69698000000001],City:"Ottawa",Type:"Museums"},
{name:"Parc Des Ancres",location:[45.330642700000006,-73.9666672],City:"Pointe-des-Cascades",Type:"Museums"},
{name:"Parkwood Estate",location:[43.904726000000004,-78.867621],City:"Oshawa",Type:"Museums"},
{name:"Parliament Interpretive Centre",location:[43.651169700000004,-79.3637298],City:"Toronto",Type:"Museums"},
{name:"Peel Art Gallery Museum + Archives",location:[43.68476941,-79.75782037],City:"Brampton",Type:"Museums"},
{name:"Penitentiary Museum",location:[44.22155413,-76.51385384],City:"Kingston",Type:"Museums"},
{name:"Perth Museum",location:[44.901140000000005,-76.25069],City:"Perth",Type:"Museums"},
{name:"Peterborough Museum & Archives",location:[44.3096324,-78.3030583],City:"Peterborough",Type:"Museums"},
{name:"Pickering Museum Village",location:[43.924929,-79.071373],City:"Greenwood",Type:"Museums"},
{name:"Pinhey's Point",location:[45.43825028,-75.95248328],City:"Dunrobin, ON",Type:"Museums"},
{name:"Redpath Sugar Museum",location:[43.64255,-79.37035999999999],City:"Toronto",Type:"Museums"},
{name:"Ripley's Believe It or Not!",location:[43.089538,-79.0772219],City:"Niagara Falls",Type:"Museums"},
{name:"Rock Legends Wax Museum",location:[43.09214782,-79.07672893],City:"Niagara Falls",Type:"Museums"},
{name:"Royal Ontario Museum",location:[43.66824413,-79.39476013],City:"Toronto",Type:"Museums"},
{name:"Sainte-Marie Among The Hurons",location:[44.73374748,-79.84438419],City:"Midland",Type:"Museums"},
{name:"Sault Ste Marie Museum",location:[46.508,-84.32791],City:"Sault Ste. Marie",Type:"Museums"},
{name:"Scarborough Museum",location:[43.75906629999999,-79.2571502],City:"Scarborough",Type:"Museums"},
{name:"Scugog Shores Historical Museum",location:[44.11231,-78.92015],City:"Port Perry",Type:"Museums"},
{name:"Spadina Historic House Museum",location:[43.67903,-79.40828990000001],City:"Toronto",Type:"Museums"},
{name:"St Catharines Museum & Welland Canal Centre",location:[43.15592,-79.19464],City:"St Catharines",Type:"Museums"},
{name:"St. Jacobs & Aberfoyle Model Railway",location:[43.53998577,-80.55191949],City:"St Jacobs",Type:"Museums"},
{name:"Stones 'n Bones Museum",location:[42.9740033,-82.4069505],City:"Sarnia",Type:"Museums"},
{name:"Sun Life Financial Museum + Arts Pass",location:[43.671878799999995,-79.3866272],City:"Toronto",Type:"Museums"},
{name:"Textile Museum of Canada",location:[43.65529341,-79.38429623],City:"Toronto",Type:"Museums"},
{name:"The Aga Khan Museum",location:[43.725390000000004,-79.33229],City:"Toronto",Type:"Museums"},
{name:"The Bata Shoe Museum",location:[43.667229999999996,-79.40011],City:"Toronto",Type:"Museums"},
{name:"The Bytown Museum",location:[45.42581914,-75.69766357],City:"Ottawa",Type:"Museums"},
{name:"The Coffee Station",location:[44.92014029,-79.37073727],City:"Gravenhurst",Type:"Museums"},
{name:"The Design Exchange",location:[43.65092876,-79.38490558],City:"Toronto",Type:"Museums"},
{name:"The Fun House",location:[43.09153,-79.07344],City:"Niagara Falls",Type:"Museums"},
{name:"The Gardiner Museum",location:[43.66811506,-79.39294207],City:"Toronto",Type:"Museums"},
{name:"The Museum",location:[43.450078399999995,-80.4895587],City:"Kitchener",Type:"Museums"},
{name:"The Niagara Apothecary",location:[43.25502,-79.07093],City:"Niagara-on-the-Lake",Type:"Museums"},
{name:"The Robert McLaughlin Gallery",location:[43.895014700000004,-78.8657042],City:"Oshawa",Type:"Museums"},
{name:"The Tree Museum",location:[44.92122,-79.26755],City:"Gravenhurst",Type:"Museums"},
{name:"Tiny Town",location:[43.7738481,-79.6176777],City:"Woodbridge",Type:"Children's Museums"},
{name:"Tom Thomson Memorial Gallery",location:[44.564409999999995,-80.94514000000001],City:"Owen Sound",Type:"Museums"},
{name:"Toronto Police Museum",location:[43.66135,-79.3851],City:"Toronto",Type:"Museums"},
{name:"Toronto Railway Museum",location:[43.640834000000005,-79.385948],City:"Toronto",Type:"Museums"},
{name:"Toronto's First Post Office & Museum",location:[43.65191,-79.3705],City:"Toronto",Type:"Museums"},
{name:"Uncle Tom's Cabin Historic Site",location:[42.58512,-82.19606],City:"Dresden",Type:"Museums"},
{name:"Victoria Memorial Square",location:[43.64305,-79.39949],City:"Toronto",Type:"Museums"},
{name:"Waterloo Region Museum",location:[43.4010807,-80.43556579999999],City:"Kitchener",Type:"Museums"},
{name:"Wheelers Pancake House And Sugar Camp",location:[44.92783,-76.53709],City:"McDonalds Corners",Type:"Museums"},
{name:"Wheelsport",location:[45.46266,-75.55053000000001],City:"Orleans",Type:"Museums"},
{name:"Whitehern Historic House & Garden",location:[43.2547799,-79.87195],City:"Hamilton",Type:"Museums"},
{name:"Windsor Public Library",location:[42.3111775,-83.0345176],City:"Windsor",Type:"Museums"},
{name:"Windsor's Community Museum",location:[42.318509999999996,-83.0425],City:"Windsor",Type:"Museums"}
];



//custom markers
var greenIcon = L.icon({
  iconUrl: '/static/images/parks.png',
  // shadowUrl: '/static/images/leaf-shadow.png',

  iconSize:     [25, 35], // size of the icon
  shadowSize:   [50, 64], // size of the shadow
  iconAnchor:   [11, 45], // point of the icon which will correspond to marker's location
  shadowAnchor: [4, 62],  // the same for the shadow
  popupAnchor:  [-3, -76] // point from which the popup should open relative to the iconAnchor
});
var plIcon = L.icon({
  iconUrl: '/static/images/historic.png',
  // shadowUrl: '/static/images/shadow.png',

  iconSize:     [25, 35], // size of the icon
  shadowSize:   [50, 64], // size of the shadow
  iconAnchor:   [11, 45], // point of the icon which will correspond to marker's location
  shadowAnchor: [4, 62],  // the same for the shadow
  popupAnchor:  [-3, -76] // point from which the popup should open relative to the iconAnchor
});
var cultIcon = L.icon({
  iconUrl: '/static/images/bulb.png',
  // shadowUrl: '/static/images/shadow.png',

  iconSize:     [25, 35], // size of the icon
  shadowSize:   [50, 64], // size of the shadow
  iconAnchor:   [11, 45], // point of the icon which will correspond to marker's location
  shadowAnchor: [4, 62],  // the same for the shadow
  popupAnchor:  [-3, -76] // point from which the popup should open relative to the iconAnchor
});


// array to store cultplaces 
var cultplaces = [];
for (var i = 0; i < cultedu.length; i++) {
// loop through parks, create markers push to cityparks
  cultplaces.push(
    L.marker(cultedu[i].location, {icon: cultIcon}).bindPopup("<h2>" + cultedu[i].name + "</h2>" + "<h3>" + cultedu[i].Type + "</h3>" )
  );
}

// array to store cityplaces 
var cityplaces = [];
for (var i = 0; i < ohsites.length; i++) {
// loop through parks, create markers push to cityparks
  cityplaces.push(
    L.marker(ohsites[i].location, {icon: plIcon}).bindPopup("<h2>" + ohsites[i].name + "</h2>" + "<h3>" + ohsites[i].Type + "</h3>" )
  );
}
// array to store cityparks markers
var cityparks = [];
for (var i = 0; i < parks.length; i++) {
// loop through parks, create markers push to cityplaces
cityparks.push(
    L.marker(parks[i].location, {icon: greenIcon}).bindPopup("<h3>" + parks[i].name + "</h3>" + "<h3>" + parks[i].Type + "</h3>")
  );
}

var _markerOnClick = function(e) {
  console.log(e);
};


// group markers to layers
var cityPrklayer = L.layerGroup(cityparks);
var cityPlclayer = L.layerGroup(cityplaces);
var cityCltlayer = L.layerGroup(cultplaces);

console.log(parks[0].name);

// add even listeners to markers
for(var i=0;i<cityparks.length;i++){
  cityparks[i].on("click",function(){
  document.getElementById("gfu").value = parks[0].name
});
}
for (var i=0;i<cityplaces.length;i++){
  cityplaces[i].on("click",function(){
  document.getElementById("gfu").value = ohsites[0].name
});
}
for (var i=0;i<cultplaces.length;i++){
  cultplaces[i].on("click",function(){
  document.getElementById("gfu").value = cultedu[0].name
});
}


// set variables to hold layers

  var street_mode = L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}", {
    attribution: "Map data &copy; <a href=\"https://www.openstreetmap.org/\">OpenStreetMap</a> contributors, <a href=\"https://creativecommons.org/licenses/by-sa/2.0/\">CC-BY-SA</a>, Imagery © <a href=\"https://www.mapbox.com/\">Mapbox</a>",
    maxZoom: 20,
    id: "mapbox.outdoors",
    accessToken: API_KEY
  })

  var treasure_mode = L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}", {
    attribution: "Map data &copy; <a href=\"https://www.openstreetmap.org/\">OpenStreetMap</a> contributors, <a href=\"https://creativecommons.org/licenses/by-sa/2.0/\">CC-BY-SA</a>, Imagery © <a href=\"https://www.mapbox.com/\">Mapbox</a>",
    maxZoom: 20,
    id: "mapbox.pirates",
    accessToken: API_KEY
  })
  
  var dark_mode = L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}", {
    attribution: "Map data &copy; <a href=\"https://www.openstreetmap.org/\">OpenStreetMap</a> contributors, <a href=\"https://creativecommons.org/licenses/by-sa/2.0/\">CC-BY-SA</a>, Imagery © <a href=\"https://www.mapbox.com/\">Mapbox</a>",
    maxZoom: 20,
    id: "mapbox.dark",
    accessToken: API_KEY
  })

// set variable to allow only one view at the time
var baseViews = {
  Outdoor_mode: street_mode,
  Pirate_mode: treasure_mode,
  Dark_mode: dark_mode
};


// controls for markers
var mpOverlay = {
  Parks: cityPrklayer,
  Places: cityPlclayer,
  Cultural: cityCltlayer

};

// Create map object add layers
var myMap = L.map("map", {
  center: [48.404508899999996,-83.522217],
  zoom: 4,
  layers: [street_mode,cityPrklayer, cityPlclayer,cityCltlayer]
});

// add layers to map control add control to map
L.control.layers(baseViews, mpOverlay).addTo(myMap);


// Function for dark mode
function toggleDarkLight() {
  var body = document.getElementById("body");
  var currentClass = body.className;
  body.className = currentClass == "dark-mode" ? "light-mode" : "dark-mode";
}


var _markerOnClick = function(e) {
  console.log("you clicked me");
 };

// TRIal and error####################################################
// var url = "http://127.0.0.1:5000/histsites";
// const parks = [];
// d3.json(url, function(response) {
//   response=[response];
//   // console.log(response);
//   // console.log(response[0].features[0].properties.mag);
//     for (var i = 0; i <= 275; i++) {
//     var location = response[0][i];
//     // console.log(location);
//     if (location) {
//       parks.push(location);
//     }
    
//   }
// });
// console.log(parks);

// var histsites = [];

// var request = new XMLHttpRequest()
// request.open('GET', 'http://127.0.0.1:5000/histsites', true)
//   request.onload = function () {
//     // Begin accessing JSON data here
//     var data = JSON.parse(this.response)
    
//     for (var i = 1; i < data.length; i++) {
//       parks[i] = {name: data[i][0], location: [data[i][6], data[i][7]]}
//     }
// };
// // Send request
// request.send()


// for (var i = 0; i < 274; i++) {
//   // console.log(ohsites.length);
// // loop through parks, create markers push to cityparks
//   cityparks.push(
//     L.marker(parks[i][0], {icon: greenIcon}).bindPopup("<h1>" + parks[i][0]+ "</h1>")
//   );
// }
// console.log(cityparks);

// console.log(ohsites);
// // array to store cityparks  markers
// var cityparks = [];
// for (var i = 0; i < ohsites.length; i++) {
// // loop through parks, create markers push to cityparks
//   cityparks.push(
//     L.marker(ohsites[i].location, {icon: greenIcon}).bindPopup("<h1>" + parks[i].name + "</h1>")
//   );


// var request = new XMLHttpRequest()
// request.open('GET', 'http://127.0.0.1:5000/histsites', true)
// request.onload = function () {
//   // Begin accessing JSON data here
//   var data = JSON.parse(this.response)

// if (request.status >= 200 && request.status < 400) {
//   data.forEach(hsite => {
//     console.log(hsite)
//   })
// } else {
//   console.log('error')
// }
// };

// var cityparks = [];
// for (var i = 0; i < parks.length; i++) {
// // loop through parks, create markers push to cityparks
//   cityparks.push(
//     L.marker(parks[i].location, {icon: greenIcon}).bindPopup("<h1>" + parks[i].name + "</h1>")
//   );
// }
// // Send request
// request.send()

    // var request = new XMLHttpRequest()
    // request.open('GET', 'http://127.0.0.1:5000/histsites', true)
    //   request.onload = function () {
    //     // Begin accessing JSON data here
    //     var data = JSON.parse(this.response)
    
    // if (request.status >= 200 && request.status < 400) {
    //   data.forEach(hsite => {
    //     parks.push({
    //       name: hsite[0],
    //       location: [hsite[6], hsite[7]]
    //     })
    //     })
    
    // } else {
    //   console.log('error')
    // }
    // };
    // // Send request
    // request.send()
    // #############################################################################